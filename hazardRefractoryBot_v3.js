...
/**
 * =====================================================================
 *  Hazard-Refractory Bot v3 — LESS RESTRICTIVE, post-spike trader
 *  File: hazardRefractoryBot_v3.js  (derived from hazardRefractoryBot.js)
 * =====================================================================
 *
 ...
 *  Run: node hazardRefractoryBot_v3.js  /  --selftest  /  --reset
 * =====================================================================
 */
...
const CONFIG = Object.freeze({
  ...
  deepBackfillTarget: 5000,
   ...
   // ── Trading v3 — per-spike, hold derived from mean + EV search (adaptive) ─
  stake: 1,
  ...
  martingaleMultiplier3: 3.2,
  martingaleMultiplier4: 3.2,
  martingaleMultiplier5: 3.2,
  martingaleMultiplier6: 3.2,
  ...
  stateFile: 'hazardBot_v3_01_state.json',
  logFile: 'hazardBot_v3_01.log',
  logLevel: 'INFO',
  telegram: {
    enabled: true,
...
function _sendEod(reason='manual'){
  ...
  let msg=`🌙 Hazard_v3 <b>DAILY REPORT — ${date}</b>\n\n`;
...
function finalizeContract(cid, poc){
  ...
  // ── CLOSE telegram — structure exactly like accuHOLD_v2 ──
  {
    ...
    const msg =
      `${emoji} <b>Hazard_v3 TRADE ${label}</b>\n\n`+
...
function runValidationSweep(){
  ...
  if(unfavorable && pOne < CONFIG.killP){
    const msg=`🛑 Hazard_v3 <b>Kill-switch: realized WR inconsistent</b>\nLast${CONFIG.validationN}: <code>${wins}/${lastN.length} ${(wins/lastN.length).toFixed(3)}</code> vs breakeven <code>${breakevenMean.toFixed(3)}</code> pOne <code>${pOne.toFixed(4)}</code>`;
    log('ERROR',msg.replace(/<[^>]+>/g,'')); telegram.send(msg); enterPaused('validation p<0.05 unfavorable');
  }
}
function checkKillSwitch(){
  if(killed) return;
  if(consecutiveLossesGlobal >= CONFIG.maxConsecutiveLosses){ const msg=`🛑 Hazard_v3 <b>Kill-switch: ${consecutiveLossesGlobal} consecutive losses</b> (limit ${CONFIG.maxConsecutiveLosses})`; log('ERROR',msg.replace(/<[^>]+>/g,'')); telegram.send(msg); enterPaused(`consecutive losses ${consecutiveLossesGlobal}`); return; }
  if(dailyLoss <= -Math.abs(CONFIG.dailyMaxLoss)){ const msg=`🛑 Hazard_v3 <b>Kill-switch: daily loss ${dailyLoss.toFixed(2)} ${CONFIG.currency}</b> (limit -${CONFIG.dailyMaxLoss})`; log('ERROR',msg.replace(/<[^>]+>/g,'')); telegram.send(msg); enterPaused(`daily loss ${dailyLoss.toFixed(2)}`); return; }
  if(dailyTrades >= CONFIG.dailyMaxTrades){ const msg=`🛑 Hazard_v3 <b>Kill-switch: daily trades ${dailyTrades}</b>`; log('ERROR',msg.replace(/<[^>]+>/g,'')); telegram.send(msg); enterPaused('daily trades limit'); return; }
}
...
async function tryTradeForAsset(client, st){
  ...
  while(buyAttempts<2){
    try{
      ...
      if(!isRace) telegram.send(`❌ Hazard_v3 <b>v3 Buy failed ${st.symbol}</b> ${msg.slice(0,120)}`);
      return;
    }
  }
  if(!buyRes) return;
  ...
  log('INFO',`Bought v3 ${st.symbol} ACCU #${cid} stake ${buyRes.buy_price} growth ${CONFIG.growthRate*100}% entryAfter ${entryAfter} hold ${holdTicks} barrier ${(barrierPct*100).toFixed(5)}%`);
  // ── OPEN telegram — structure exactly like accuHOLD_v3 TRADE OPENED ──
  {
  ...
  const entry={
    ...
    outcome:null, profit:null, contractId: cid, v3:true, entryReason, stake: parseFloat(buyRes.buy_price||martingaleStake), martingaleLevel, martingaleStake, martingaleMultiplier: CONFIG.martingaleMultiplier, buyTime: Date.now(),
  };
...
function runSelfTest(){
  console.log('Running selftest v3...');
  ...
  // v3 specific: post-spike param calc
...
async function main(){
  ...
  for(const sym of CONFIG.assets){
    ...
    if(st.history.length>=CONFIG.detection.baselineWindow){
      ...
      if(st.intervals.length>=CONFIG.calibrationMinIntervals){
        ...
        telegram.send(`🔄 Hazard_v3 <b>v3 Calibrating ${sym}</b> intervals <code>${st.intervals.length}/${CONFIG.calibrationMinIntervals}</code>`);
      }
      // v3: schedule initial entry if already ACTIVE
  ...
  // v3 main loop: poll every 800ms — analysis always runs, trading gated by pause/DOW/daily caps
  setInterval(async ()=>{
    if(killed) return;
    if(manualRestartRequired) return;
    ...
    { const todayTrades = statsManager.todayTrades(); const pnl = todayTrades.reduce((s,t)=>s+Number(t.profit||0),0); if(todayTrades.length >= CONFIG.dailyMaxTrades || pnl <= -CONFIG.dailyMaxLoss){ dailyHardStop=true; if(!_dailyStopNotified){ _dailyStopNotified=true; const msg=`⛔ Hazard_v3 <b>Daily hard stop</b>\n${todayTrades.length} trades, net ${money(pnl,currencyStr())}.\nPaused until next UTC day.`; log('WARN', msg.replace(/<[^>]+>/g,'')); telegram.send(msg); } } else { _dailyStopNotified=false; } }
  ...
  log('INFO','Bot v3 running — post-spike trades every spike');
  log('INFO', martingaleInfoLine()+' | '+lossStreakInfoLine());
  {
    ...
    telegram.send(
      `<b>hazardRefractoryBot v3 — Online</b>\n\n`+
