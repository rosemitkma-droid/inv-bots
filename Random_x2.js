...
// ── asset selection / suspension ──────────────────────────────────
    suspendAssetAfterTrade: boolEnv('SUSPEND_AFTER_TRADE', false),   // suspend traded asset after each trade
    singleActiveAsset:      boolEnv('ONE_ASSET_AT_A_TIME', true),     // subscribe + trade ONLY ONE random asset per trade
    ...
    stateFile: strEnv('STATE_FILE', 'randomRepeatX2_state.json'),
...
class RandomRepeatTradingBot {
    ...
    async handleAuthorized() {
        ...
        // Every reconnect is a NEW trade: drop the previous cycle's asset and
        // randomly select ONE fresh asset to subscribe to and trade. If a trade
        // is already open it is reconciled separately before any new pick.
        ...
        const assetsLine = '🎲 This trade: <code>' + htmlEscape(this.activeAsset || '?') + '</code> (random pick per trade) | pool: <code>' + this.assets.join(', ') + '</code>';

        telegram.send(
            '🤖 <b>Random_x2 Repeat Bot — ONLINE</b>\n' +
    ...
    async initializeSubscriptions() {
        // Hardwired single-asset mode: subscribe ONLY to the one randomly
        // selected asset for this trade (one ticks_history + one ticks stream).
        const targets = this.activeAsset ? [this.activeAsset] : [];
        console.log(`Subscribing to 1 asset for this trade: ${targets.join(', ')}...`);
    ...
    handleTickUpdate(tick) {
        ...
        // Only the ONE randomly selected asset is subscribed and traded; ignore
        // anything else that might arrive on the wire.
        ...
        // One randomly selected asset per trade — only ever trade the asset we
        // subscribed to this cycle.
        if (this.activeAsset) this.analyzeTicks(this.activeAsset);
    }

    // Single-asset mode: choose ONE random asset to subscribe to and
    ...
    analyzeTicks(asset) {
        ...
        // Random_x2 "digit repeat" strategy:
        // predicted digit = the CURRENT last digit (bet that it repeats on the
        // next tick). The trade will lose if the preceding digit already equals
        // the predicted digit (a repeat just happened) — skip that case.
        const predictedDigit = history[history.length - 1];
        const precedingDigit = history.length >= 2 ? history[history.length - 2] : null;

        if (precedingDigit === predictedDigit) {
            console.log(`[${asset}] Skipping trade for digit: ${predictedDigit} | preceding: ${precedingDigit} === predicted (repeat already happened — likely loss)`);
            return;
        }
    ...
    async placeTrade(asset, predictedDigit) {
        ...
        try {
            ...
            const proposalRes = await this.client._send({
                ...
                basis: 'stake',
                contract_type: 'DIGITDIFF', // win when the last digit DIFFERS (next tick != predicted digit)
    ...
    handleTradeResult(contract) {
        ...
        if (!this.endOfDay) {
            ...
            setTimeout(() => {
                this.tradeInProgress = false;
                this.Pause = false;
                // Trade confirmed settled — DROP the selected asset and start a fresh random
                // pick for the NEXT trade (handleAuthorized re-selects on reconnect).
    ...
    _sendHourly() {
        ...
        if (!h.trades) {
            telegram.send(
                '⏰ <b>Random_x2 Repeat Bot — Hourly Summary GMT (' + date + ' ' + pad(hour) + ':00-' + pad(hour) + ':59)</b>\n\n' +
                'No trades this hour.\n\n' +
    ...
    sendTelegramSummary() {
        ...
        const summaryText =
            '📊 <b>Random_x2 Repeat Bot — Summary</b>\n' +
    ...
    sendTradeResultNotification(asset, contract, won, profit) {
        ...
        const summaryText =
            (won ? '✅ <b>Random_x2 Repeat Bot — WIN</b>' : '❌ <b>Random_x2 Repeat Bot — LOSS</b>') + '\n\n' +
            'Trade #' + tradeId + '\n' +
            'Asset: <code>' + htmlEscape(asset) + '</code>\n' +
    ...
    sendErrorNotification(errorMessage) {
        telegram.send(
            '🚨 <b>Random_x2 Repeat Bot — Error</b>\n\n' +
    ...
    _sendEod() {
        ...
        let msg = '🌙 <b>Random_x2 Repeat Bot — END OF TRADE DAY</b>\n' +
    ...
    start() {
        console.log('Asset mode: ONE random asset per trade (subscribe 1 → trade 1 → drop on disconnect → re-pick on reconnect)');
...
// Random_x2 — digit-REPEAT strategy: barrier = current last digit (DIGITDIFF),
// skip when the preceding digit equals it. Each trade = 1 randomly selected
// asset subscribed + traded, dropped on disconnect, re-picked on reconnect.
// Note: DIGITDIFF hit rate ≈ 1 in 10 (≈9x payout) vs the differ bot's 9 in 10
// (≈1.1x), so martingale escalates fast — keep loss caps conservative.
const bot = new RandomRepeatTradingBot({
    initialStake: 0.61,
    multiplier: 11.3,
    ...
    // singleActiveAsset is HARDWIRED to true (one random asset per trade).
