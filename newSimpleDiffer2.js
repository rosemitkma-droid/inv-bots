...
const CONFIG = Object.freeze({
  ...
  assets: ['R_75'], //'1HZ10V','1HZ25V','1HZ50V','1HZ75V','1HZ100V','R_10','R_25','R_50','R_75','R_100','RDBULL','RDBEAR'
  ...
  // Persistence/logging
  stateFile: strEnv('STATE_FILE', 'simpleDiffer2_state_01.json'),
  logFile: strEnv('LOG_FILE', 'simpleDiffer2_bot_01.log'),
...
class TradingBot {
  ...
  async _onAuthorized(info) {
    ...
    telegram.send(
      `🤖 <b>SIMPLE2 Digit Differ Bot Online</b>\n\n` +
  ...
  _onDisconnected(code, reason, wasAuthorized) {
    telegram.send(`⚠️ <b>SIMPLE2 Connection lost</b>\ncode: <code>${code}</code>\nwas authorized: ${wasAuthorized ? 'yes' : 'no'}\n🔄 reconnecting...`);
  ...
  _onTradeOpen(t) {
    ...
    const a = t.analysis || {};
    telegram.send(
      `🟢 <b>TRADE OPENED — SIMPLE2 DIGIT DIFFER</b>\n\n` +
  ...
  _onTradeResult(t) {
    ...
    if (won0) {
      ...
      telegram.send(
        `🛑 <b>SIMPLE2 CIRCUIT BREAKER TRIPPED</b>\n\n` +
    ...
    telegram.send(
      `${emoji} <b>TRADE ${label} — SIMPLE2 DIGIT DIFFER</b>\n\n` +
      `🎫 Contract: <code>#${t.contractId}</code>\n` +
  ...
  async _recoverStuckTrade(reason) {
    ...
    telegram.send(
      `<b>SIMPLE2 STUCK TRADE RECOVERED [${reason}]</b>\n` +
      `Contract: ${contractId}\n` +
      `Asset: ${symbol}\n` +
  ...
  _sendHourly() {
    ...
    if (!list.length) {
      telegram.send(`⏰ <b>SIMPLE2 Hourly Summary GMT (${date} ${pad(hour)}:00-${pad(hour)}:59)</b>\n\nNo trades this hour.\n\n💼 Overall Profit: ${money(this.stats.overallProfit, this.currency())}`);
      return;
    }
    let msg = `⏰ <b>SIMPLE2 Hourly Summary GMT (${date} ${pad(hour)}:00-${pad(hour)}:59)</b>\n\n` +
  ...
  _sendEod(reason = 'manual') {
    ...
    let msg = `🌙 <b>SIMPLE2 END OF TRADE DAY — GMT</b>\n` +
    ...
    msg += `<b>──SIMPLE2 Overall / Stored Stats ──</b>\n` +
           `💼 Overall Profit: <b>${money(this.stats.overallProfit, this.currency())}</b>\n` +
    ...
    if (this.cfg.calibEnabled) {
      ...
      if (keys.length) {
        msg += `<b>──SIMPLE2 Symbol Calibration (rolling ${this.cfg.calibWindow}) ──</b>\n`;
  ...
  stop(signal) {
    ...
    telegram.send(`🛑 <b>SIMPLE2 Differ stopped</b>\nSignal: ${htmlEscape(signal)}\n💼 Overall Profit: ${money(this.stats.overallProfit, this.currency())}`);
    if (this._analysisT) clearInterval(this._analysisT);
