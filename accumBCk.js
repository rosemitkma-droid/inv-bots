...
// FILE PATHS
// ============================================================
const STATE_FILE        = path.join(__dirname, 'accumulator_bot-01_v1_state.json');
const HISTORY_FILE      = path.join(__dirname, 'accumulator_bot-01_v1_history.json');
...
class TelegramService {
    ...
    static async sendTradeAlert(type, symbol, stake, profitDetails = {}) {
        ...
        const lines = [
            `${emoji} <b>ACCUMULATOR BOT v1 — ${type}</b>`,
            `Asset: <b>${symbol}</b>`,
...
class AccumulatorBot {
    ...
    async start() {
        ...
        await TelegramService.sendMessage(
            `🤖 <b>ACCUMULATOR BOT v1.1 STARTED</b>\n` +
        ...
        setInterval(() => {
            ...
            if (lastWinAsset && !state.session.endedByTime) {
                if (hour >= CONFIG.DISCONNECT_HOUR && mins >= 0) {
                    ...
                    TelegramService.sendMessage(`🌙 <b>Time Stop</b> — ${CONFIG.DISCONNECT_HOUR}:00 reached after win. Bot paused.`);
                    this.stop();
                }
            }
