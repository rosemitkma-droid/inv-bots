...
/**
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║      DERIV SYNTHETIC INDICES CALLE/PUTE BOT  —  v2.0  "SAFE PROFIT PRO"   ║
 * ║  STRATEGY:  ADVANCED TECHNICAL CONFLUENCE (3-layer filter)               ║
 * ║  ──────────────────────────────────────────────────────────────────────  ║
 * ║  Operates in the Volatility market (R_100 / R_10) using three layers:    ║
 * ║    1. TREND        → EMA fast vs EMA slow (macro direction)              ║
 * ║    2. MOMENTUM     → RSI (avoid overbought / oversold areas)             ║
 * ║    3. CONFIRMATION → MACD (validates movement strength before purchase)  ║
 * ║  Trades ONLY in clear trending markets (no range chop).                  ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
 */
...
// FILE PATHS
// ============================================================
const STATE_FILE = path.join(__dirname, 'safeProfitPro-state_01.json');
const HISTORY_FILE = path.join(__dirname, 'safeProfitPro-history_01.json');
...
const CONFIG = {
    ...
    // ── Staking (SAFE PROFIT PRO recommended) ────────────────
    INITIAL_STAKE: 0.35,           // USD recommended by strategy
    INVESTMENT_AMOUNT: 152,        // Bankroll / capital pool
    MARTINGALE_MULTIPLIER: 1.0,    // 1.0 => flat staking (safe), set >1.0 to enable martingale
    MAX_MARTINGALE_LEVEL: 0,       // 0 => no recovery escalation
    AFTER_MAX_LOSS: 'reset',
    CONTINUE_EXTRA_LEVELS: 8,
    EXTRA_LEVEL_MULTIPLIERS: [2.1, 2.2, 2, 2.1, 2.2, 2.3, 2.3],
    AUTO_COMPOUNDING: false,
    COMPOUND_PERCENTAGE: 0.1,

    // ── Session risk (SAFE PROFIT PRO recommended) ───────────
    TAKE_PROFIT_PERCENT: 10,       // 10% of bankroll per session
    STOP_LOSS_PERCENT: 15,         // 15% of bankroll per session
    COOLDOWN_CANDLES: 5,

    // ── SAFE PROFIT PRO — TECHNICAL INDICATOR PARAMETERS ─────
    INDICATORS: {
        // Layer 1 — TREND (EMA confluence for macro direction)
        EMA_FAST: 20,
        EMA_SLOW: 50,
        // Layer 2 — MOMENTUM (RSI to avoid overbought / oversold)
        RSI_PERIOD: 14,
        RSI_OVERSOLD: 30,
        RSI_OVERBOUGHT: 70,
        RSI_UPPER_CALLE: 65,        // For CALLE require RSI <= 65 (not overbought)
        RSI_LOWER_PUTE: 35,         // For PUTE require RSI >= 35 (not oversold)
        // Layer 3 — CONFIRMATION (MACD movement strength)
        MACD_FAST: 12,
        MACD_SLOW: 26,
        MACD_SIGNAL: 9,
    },
    ...
    DURATION: 5,
    DURATION_UNIT: 'm',
    MIN_CANDLES_REQUIRED: 70,      // enough data for EMA50 + MACD warm-up
    ...
    // ── Active Index Assets (Volatility market) ──────────────
    ACTIVE_ASSETS: [
        'R_10',
        // 'R_100',
    ],
...
// ============================================================
// TECHNICAL INDICATORS  [NEW — SAFE PROFIT PRO]
// ============================================================
...
// ============================================================
// STAKE CALCULATOR — flat by default (SAFE PROFIT PRO)
// ============================================================
class StakeCalculator {
    ...
    static calculate(investmentRemaining, martingaleLevel = 0) {
        ...
        // SAFE mode: multiplier <= 1.0 (or level 0) => always flat base stake
        if (CONFIG.MARTINGALE_MULTIPLIER <= 1.0 || level === 0) {
            ...
            if (data.assets) {
                Object.keys(data.assets).forEach(symbol => {
                    if (state.assets[symbol]) {
                        ...
                        LOGGER.info(`${symbol}: Rec=${a.recoveryStep} Stake=$${(a.currentStake || 0).toFixed(2)} P/L=$${(a.netPL || 0).toFixed(2)} | Wins=${a.winsCount} Losses=${a.lossesCount} Trades=${a.tradesCount}`);
                    }
                });
            }
...
class TelegramService {
    ...
    static async sendTradeAlert(type, symbol, direction, stake, duration, durationUnit, details = {}) {
        ...
        const lines = [
            `${emoji} <b>SAFE PROFIT PRO — ${type}</b>`,
        ...
        if (type === 'OPEN' && details.signal) {
            ...
            const ind = sig.ind || {};
            lines.push(`\u{1f9e0} <b>Signal:</b> SAFE PROFIT PRO — Technical Confluence`);
            lines.push(`Trend: ${sig.trendLabel} (EMA${CONFIG.INDICATORS.EMA_FAST} vs EMA${CONFIG.INDICATORS.EMA_SLOW})`);
            lines.push(`RSI: ${ind.rsi ?? '—'} | MACD Hist: ${ind.histogram ?? '—'}`);
    ...
    static async sendSessionSummary() {
        ...
        await this.sendMessage([
            `\u{1f4ca} <b>SAFE PROFIT PRO SESSION SUMMARY</b>`,
    ...
    static async sendStartupMessage() {
        ...
        await this.sendMessage([
            `\u{1f916} <b>SAFE PROFIT PRO STARTED</b>`,
            `Strategy: Advanced Technical Confluence (EMA + RSI + MACD)`,
            `Trend: EMA${CONFIG.INDICATORS.EMA_FAST}/${CONFIG.INDICATORS.EMA_SLOW} | RSI${CONFIG.INDICATORS.RSI_PERIOD} | MACD(${CONFIG.INDICATORS.MACD_FAST},${CONFIG.INDICATORS.MACD_SLOW},${CONFIG.INDICATORS.MACD_SIGNAL})`,
            `Stake: $${CONFIG.INITIAL_STAKE} | TP: ${CONFIG.TAKE_PROFIT_PERCENT}% | SL: ${CONFIG.STOP_LOSS_PERCENT}%`,
            `Risk: ${CONFIG.MARTINGALE_MULTIPLIER <= 1.0 ? 'Flat staking (safe)' : 'Martingale progression'}`,
            `Capital: $${state.capital.toFixed(2)}`,
            TradingSessionManager.getStatusString(),
            ``,
...
// ============================================================
// SESSION MANAGER  [SAFE PROFIT PRO — % bankroll risk]
// ============================================================
class SessionManager {

    static isSessionActive() { return state.session.isActive; }

    static getSessionRisk() {
        const bankroll = state.session.startCapital || CONFIG.INVESTMENT_AMOUNT;
        return {
            bankroll,
            profitTarget: bankroll * CONFIG.TAKE_PROFIT_PERCENT / 100,
            stopLoss: -bankroll * CONFIG.STOP_LOSS_PERCENT / 100,
        };
    }
    ...
    static checkSessionTargets() {
        ...
        if (netPL >= risk.profitTarget) {
            LOGGER.trade(`SAFE PROFIT PRO: Session take-profit reached (+$${risk.profitTarget.toFixed(2)} = ${CONFIG.TAKE_PROFIT_PERCENT}% of $${risk.bankroll.toFixed(2)})`);
            this.endSession('PROFIT_TARGET');
            return true;
        }

        if (netPL <= risk.stopLoss) {
            LOGGER.error(`SAFE PROFIT PRO: Session stop-loss reached ($${risk.stopLoss.toFixed(2)} = ${CONFIG.STOP_LOSS_PERCENT}% of $${risk.bankroll.toFixed(2)})`);
    ...
    static checkDayChange() {
        ...
        if (state.currentTradeDay && state.currentTradeDay !== today) {
            ...
            TelegramService.sendMessage(
                `\u{1f319} <b>SAFE PROFIT PRO END OF DAY ${state.currentTradeDay}</b>\nP/L: $${(dayStats?.netPL || 0).toFixed(2)}\nCapital: $${state.capital.toFixed(2)}`
            );
    ...
    static recordTradeResult(symbol, profit, direction, stake) {
        ...
        // Credit stake + profit back to rolling pool
        ...
        if (profit > 0) {
            ...
            if (a.consecutiveLosses >= 10) {
                ...
                TelegramService.sendMessage(
                    `❄️ <b>[${symbol}] SAFE PROFIT PRO COOL-DOWN ACTIVATED</b>\n` +
...
class ConnectionManager {
    ...
    // OHLC handler — on candle close, run SAFE PROFIT PRO confluence
    handleOHLC(ohlc) {
    ...
    onClose() {
        ...
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
            ...
            TelegramService.sendMessage(`⚠️ <b>SAFE PROFIT PRO CONNECTION LOST</b> — Reconnecting (attempt ${this.reconnectAttempts})`);
            ...
            TelegramService.sendMessage(`\u{1f6d1} <b>SAFE PROFIT PRO STOPPED</b> — Max reconnections\nFinal P/L: $${(state.session.netPL || 0).toFixed(2)}`);
            process.exit(1);
        }
    }
...
// ============================================================
// MAIN BOT CLASS — SAFE PROFIT PRO (Advanced Technical Confluence)
// ============================================================
class IndexBot {
    ...
    async start() {
        console.log('\n' + '═'.repeat(74));
        console.log(' DERIV CALLE/PUTE BOT v2.0 — SAFE PROFIT PRO (Technical Confluence)');
        ...
        console.log(`Indicators: EMA${CONFIG.INDICATORS.EMA_FAST}/${CONFIG.INDICATORS.EMA_SLOW} + RSI${CONFIG.INDICATORS.RSI_PERIOD} + MACD(${CONFIG.INDICATORS.MACD_FAST},${CONFIG.INDICATORS.MACD_SLOW},${CONFIG.INDICATORS.MACD_SIGNAL})`);
        console.log(`Risk      : ${CONFIG.MARTINGALE_MULTIPLIER <= 1.0 ? 'FLAT STAKING (safe)' : 'Martingale'} | Stake $${CONFIG.INITIAL_STAKE} | TP ${CONFIG.TAKE_PROFIT_PERCENT}% | SL ${CONFIG.STOP_LOSS_PERCENT}%`);
        ...
        state.session.startCapital = state.capital;   // bankroll basis for % targets
        ...
        LOGGER.info('SAFE PROFIT PRO v2.0 fully started!');
    }
    ...
    // ════════════════════════════════════════════════════════
    // CORE TRADE LOGIC — SAFE PROFIT PRO confluence on candle close
    // ════════════════════════════════════════════════════════
    processNewCandle(symbol, lastClosedCandle) {
        ...
        // ── SAFE PROFIT PRO: 3-layer technical confluence ──
        const signal = this._evaluateConfluence(a.closedCandles);
        if (!signal) {
            LOGGER.debug(`[${symbol}] No SAFE PROFIT PRO confluence signal — no trade`);
            return;
        }

        const direction = signal.direction;
        a.lastTradeDirection = direction;
        a.currentDirection = direction;

        LOGGER.signal(
            `[${symbol}] SAFE PROFIT PRO ${direction === 'CALLE' ? '\u{1f4c8} CALLE' : '\u{1f4c9} PUTE'} — ` +
            `${signal.trendLabel} trend | RSI ${signal.ind.rsi} | MACD hist ${signal.ind.histogram}`
        );

        this._executeBuy(symbol, direction, stake, {
            method: 'SAFE_PROFIT_PRO_CONFLUENCE',
            reason: signal.reason,
            trendLabel: signal.trendLabel,
            marketMode: signal.trendLabel,
            ind: signal.ind,
        });
    }

    // ── SAFE PROFIT PRO — Advanced Technical Confluence engine ──
    // 1. TREND:        EMA fast vs EMA slow (macro direction)
    // 2. MOMENTUM:     RSI avoids overbought / oversold
    // 3. CONFIRMATION: MACD line vs signal line + histogram strength
    _evaluateConfluence(closedCandles) {
        const I = CONFIG.INDICATORS;
        if (!closedCandles || closedCandles.length < CONFIG.MIN_CANDLES_REQUIRED) return null;

        const closes = closedCandles.map(c => c.close);
        const last = closes.length - 1;

        const emaFast = TechnicalIndicators.ema(closes, I.EMA_FAST);
        const emaSlow = TechnicalIndicators.ema(closes, I.EMA_SLOW);
        const rsi = TechnicalIndicators.rsi(closes, I.RSI_PERIOD);
        const macd = TechnicalIndicators.macd(closes, I.MACD_FAST, I.MACD_SLOW, I.MACD_SIGNAL);
        ...
        const signalLine = macd.signalLine[last];
        const hist = macd.histogram[last];

        const ind = {
            emaFast: +fast.toFixed(5),
            emaSlow: +slow.toFixed(5),
            rsi: +rsi.toFixed(2),
            macd: +macdLine.toFixed(5),
            macdSignal: +signalLine.toFixed(5),
            histogram: +hist.toFixed(5),
        };

        const bullTrend = fast > slow;
        const bearTrend = fast < slow;

        // ── CALLE (Rise) ──
        // Trend up + RSI not overbought (momentum) + MACD bullish (confirmation)
        if (bullTrend) {
            const momentumOk = rsi >= I.RSI_OVERSOLD && rsi <= I.RSI_UPPER_CALLE;
            const confirmOk = macdLine > signalLine && hist > 0;
            if (momentumOk && confirmOk) {
                return {
                    direction: 'CALLE',
                    trendLabel: 'BULLISH',
                    reason: `Uptrend EMA${I.EMA_FAST}>EMA${I.EMA_SLOW} + RSI ${rsi.toFixed(1)} (not overbought) + MACD hist ${hist.toFixed(4)} (confirming)`,
                    ind,
                };
            }
        }
        ...
        // ── PUTE (Fall) ──
        // Trend down + RSI not oversold (momentum) + MACD bearish (confirmation)
        if (bearTrend) {
            const momentumOk = rsi >= I.RSI_LOWER_PUTE && rsi <= I.RSI_OVERBOUGHT;
            const confirmOk = macdLine < signalLine && hist < 0;
            if (momentumOk && confirmOk) {
                return {
                    direction: 'PUTE',
                    trendLabel: 'BEARISH',
                    reason: `Downtrend EMA${I.EMA_FAST}<EMA${I.EMA_SLOW} + RSI ${rsi.toFixed(1)} (not oversold) + MACD hist ${hist.toFixed(4)} (confirming)`,
                    ind,
                };
            }
        }
    ...
    _executeBuy(symbol, direction, stake, signalInfo = {}) {
        ...
        // Deduct stake from rolling investment pool
    ...
    _recoverStuckTrade(reason) {
        ...
        TelegramService.sendMessage(
            `⚠️ <b>SAFE PROFIT PRO STUCK TRADE RECOVERED [${reason}]</b>\n` +
            `Contract: ${contractId}\n` +
...
console.log('\n\u{1f680} Starting SAFE PROFIT PRO v2.0...\n');
...
const statusInterval = setInterval(() => {
    ...
    CONFIG.ACTIVE_ASSETS.forEach(sym => {
        ...
        if (p) {
            ...
            pairLines += `\n  ${sym}: Rec${p.recoveryStep} $${(p.currentStake || 0).toFixed(2)} | ${p.trades}t ${p.wins}W/${p.losses}L $${(p.netPL || 0).toFixed(2)} | Pos:${p.activePositions} ${normal}${cdwn}`;
        }
    });
