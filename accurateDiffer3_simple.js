...
/**
 * =====================================================================
 *  Deriv Digit Differ Trading Bot (Simplified Edition)
 * =====================================================================
 *
...
const CONFIG = Object.freeze({
  ...
  // ── Persistence ────────────────────────────────────────────────
  stateFile: strEnv('STATE_FILE', 'accurateDiffer3_simple07_state.json'),
  logFile:   strEnv('LOG_FILE', 'accurateDiffer3_simple07.log'),
...
class TradingBot {
  ...
  async start() {
    logger.info('===== Simple Digit Bot starting =====');
  ...
  _onTradeOpen(t) {
    ...
    telegram.send(
      ...
      `• Loss probability upper bound: <b>${(a.pUpper * 100).toFixed(2)}%</b>\n` +
  ...
  _sendHourly() {
    ...
    if (!list.length) {
      telegram.send(`⏰ <b>Simple Digit Hourly Summary GMT (${date} ${pad(hour)}:00-${pad(hour)}:59)</b>\n\nNo trades this hour.\n\n💼 Overall Profit: ${money(this.stats.overallProfit, this.currency())}`);
      return;
    }
    let msg = `⏰ <b>Simple Digit Hourly Summary GMT (${date} ${pad(hour)}:00-${pad(hour)}:59)</b>\n\n` +
  ...
  _sendEod(reason = 'manual') {
    ...
    let msg = `🌙 <b>Simple Digit END OF TRADE DAY — GMT</b>\n` +
...
class DifferBacktester {
  constructor(cfg, client, market) {
    ...
    this.client = client;
    this.market = market;
    this.analyzer = new DigitAnalyzer(cfg);
  }
  ...
  async run(symbols) {
    ...
    console.log(
      ...
      `          analysisWindow=${this.cfg.analysisWindow}  duration=${this.cfg.durationTicks}t\n` +
