...
// FILE PATHS  [RETAINED]
// ============================================================
const STATE_FILE = path.join(__dirname, 'will4_09-state.json');
const HISTORY_FILE = path.join(__dirname, 'will4_09-history.json');
...
// ============================================================
// TECHNICAL INDICATORS — WPR ONLY (FIXED)
// ============================================================
class TechnicalIndicators {
    ...
    static calculateWPR(candles, period = 80) {
        ...
        try {
            ...
            // FIX: Use proper .map() method call instead of broken [validCandles.map]
...
// ============================================================
// SIGNAL MANAGER — WPR-based signal detection (FIXED)
// ============================================================
class SignalManager {
    ...
    /**
     * Check for BUY signal — WPR crosses above -20 (must have visited -80 first)
     */
    static checkBuySignal(symbol) {
        ...
        // BUY: WPR crosses ABOVE -20 (from below -20)
    ...
    /**
     * Check for SELL signal — WPR crosses below -80 (must have visited -20 first)
     */
    static checkSellSignal(symbol) {
        ...
        // SELL: WPR crosses BELOW -80 (from above -80)
...
class ConnectionManager {
        ...
        if (isNewCandle) {
            ...
            if (closed.open_time !== a.lastProcessedCandleOpenTime) {
                ...
                if (!alreadyIn) {
                    ...
                    // FIX: Update WPR on candle close (only once per candle)
                    // This is the authoritative WPR update - remove duplicate in processNewCandle
    ...
    handleCandlesHistory(r) {
        ...
        // FIX: Initialize WPR properly from loaded candles
        if (a.closedCandles.length >= CONFIG.WPR_PERIOD) {
            // Don't use prevWpr from state here - it will be updated on next candle
            a.wpr = TechnicalIndicators.calculateWPR(a.closedCandles, CONFIG.WPR_PERIOD);
            // Reset prevWpr to wpr to avoid false crosses on initial load
            a.prevWpr = a.wpr;
        }
...
class IndexBot {
    ...
    processNewCandle(symbol, lastClosedCandle) {
        ...
        // FIX: REMOVED duplicate WPR update - now only happens in handleOHLC
        // This was causing WPR to update twice per candle, corrupting prevWpr
        ...
        if (a.forceRecoverDirection) {
            ...
            const dir3 = a.forceRecoverDirection === 'CALLE' ? 'PUTE' : 'CALLE';
