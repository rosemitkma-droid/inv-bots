...
// ============================================
const STATE_FILE = path.join(__dirname, 'KriseFallM_2n_010102-state.json');
const HISTORY_FILE = path.join(__dirname, 'KriseFallM_2n_010102-history.json');
const MAXSTREAK_FILE = path.join(__dirname, 'KriseFallM_2n_010102-maxstreak.json');
...
class AssetMaxStreakManager {
    ...
    async computeAllMaxStreaks(connection) {
        ...
        await TelegramService.sendMessage(
            '🔄 <b>RISEFALL2n MaxStreak Update Started</b>\n' +
        ...
        await TelegramService.sendMessage(
            '✅ <b>RISEFALL2n MaxStreak Update Complete</b>\n' +
            `${summary}\n` +
            'Trading has RESUMED.'
        );
    }
...
class TelegramService {
    ...
    static async sendTradeAlert(type, symbol, direction, stake, duration, durationUnit, details = {}) {
        ...
        // Get assetMaxStreak for display
        const assetMaxStreak = assetMaxStreakManager ? assetMaxStreakManager.getMaxStreak(symbol) : 'N/A';
        const currentStreak = assetState?.closedCandles?.length >= 2
            ? AlternatingRegimeDetector.getCurrentStreak(assetState.closedCandles)
            : 0;

        const message = `
${emoji} <b>${type} TRADE ALERT 2n</b>
Asset: ${symbol}
Direction: ${direction}
Stake: $${stake.toFixed(2)}
Duration: ${duration} (${durationUnit === 't' ? 'Ticks' : durationUnit === 's' ? 'Seconds' : 'Minutes'})
Martingale Level: ${assetMartingale}
MaxStreak: ${assetMaxStreak} | CurrentStreak: ${currentStreak}
...
`
        ...
        try {
            ...
            ACTIVE_ASSETS.forEach(symbol => {
                ...
                if (a && a.tradesCount > 0) {
                    ...
                    assetBreakdown += `\n  ${symbol}: ${a.tradesCount} trades, ${a.winsCount}W/${a.lossesCount}L (${winRate}%), P/L: $${a.netPL.toFixed(2)}, Mart: ${a.martingaleLevel}, MaxStreak: ${ms}`;
                }
            });
    ...
    static async sendDayEndSummary(dateKey) {
        try {
            ...
            const message = [
                `🌙 <b>END OF DAY REPORT 2n - ${dateKey}</b>`, ``,
    ...
    static async sendStartupMessage() {
        try {
            ...
            const message = [
                `🤖 <b>DERIV RISE/FALL BOT STARTED 2n</b>`,
        ...
        try {
            ...
            ACTIVE_ASSETS.forEach(symbol => {
                ...
                if (a) {
                    const ac = getAssetConfig(symbol);
                    const ms = assetMaxStreakManager ? assetMaxStreakManager.getMaxStreak(symbol) : 'N/A';
                    assetInfo += `\n  ${symbol} (${ac.TIMEFRAME_LABEL}/${ac.DURATION}${ac.DURATION_UNIT}): Mart=${a.martingaleLevel}, Stake=$${a.currentStake.toFixed(2)}, P/L=$${a.netPL.toFixed(2)}, MaxStreak=${ms}`;
                }
            });

            const message = [
                `⏰ <b>Rise/Fall Bot Hourly Summary 2n</b>`, ``,
                `📊 <b>Last Hour</b>`,
...
// ============================================
// ALTERNATING REGIME DETECTOR (Simplified)
// ============================================
/**
 * SIMPLIFIED: Only computes maxStreak and currentStreak from
 * the live closedCandles (which is kept at ≤50 candles).
 * The authoritative maxStreak is stored in assetMaxStreakManager.
 */
class AlternatingRegimeDetector {

    /**
     * Computes currentStreak from the tail of the candle sequence.
     * This is the only metric needed at trade time.
     */
    static getCurrentStreak(candleHistory) {
        if (!Array.isArray(candleHistory) || candleHistory.length < 2) return 0;
        const seq = candleHistory.map(c => (c.close > c.open ? 1 : 0));
        return this._tailStreak(seq);
    }

    /**
     * Analyzes closedCandles and returns ONLY maxStreak + currentStreak.
     * Used for logging/display — NOT for the trade trigger
     * (the trigger uses assetMaxStreakManager.getMaxStreak()).
     */
    static analyze(candleHistory) {
        if (!Array.isArray(candleHistory) || candleHistory.length < 2) {
            return { maxStreak: 0, currentStreak: 0 };
        }
        const seq = candleHistory.map(c => (c.close > c.open ? 1 : 0));
        const { maxStreak, currentStreak } = this._streakAnalysis(seq);
        return { maxStreak, currentStreak };
    }

    // ── Internal helpers ──────────────────────────────────────────
    static _streakAnalysis(seq) {
        if (seq.length < 2) return { maxStreak: 1, currentStreak: 1 };
        let maxStreak = 1, cur = 1;
        for (let i = 1; i < seq.length; i++) {
            if (seq[i] !== seq[i - 1]) { cur++; if (cur > maxStreak) maxStreak = cur; }
            else cur = 1;
        }
        return { maxStreak, currentStreak: this._tailStreak(seq) };
    }

    static _tailStreak(seq) {
...
class ConnectionManager {
    ...
    handleBuyResponse(response) {
        ...
        if (position && foundSymbol) {
            // Get current streak for alert display
            const assetState = state.assets[foundSymbol];
            const { currentStreak } = AlternatingRegimeDetector.analyze(assetState.closedCandles);
            const assetMaxStreak = assetMaxStreakManager ? assetMaxStreakManager.getMaxStreak(foundSymbol) : 'N/A';
    ...
    handleCandlesHistory(response) {
        ...
        LOGGER.info(`  ✅ Ready for trade analysis (lookback 50 candles)`);
    }
    ...
    onClose() {
        ...
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
            ...
            TelegramService.sendMessage(
                `⚠️ <b>CONNECTION LOST - RECONNECTING 2n</b>\n` +
            ...
            TelegramService.sendMessage(`🛑 <b>BOT STOPPED 2n</b>\nMax reconnection attempts reached.\nFinal P&L: $${state.session.netPL.toFixed(2)}`);
            process.exit(1);
        }
    }
...
class DerivBot {
    ...
    async start() {
        ...
        console.log(`  Strategy: currentStreak >= (assetMaxStreak - 5) to trigger trade`);
        console.log(`  MaxStreak Source: 50,000 candles (10 × 5,000 batches)`);
    ...
    executeRecoveryTrade(symbol, closedCandle) {
        ...
        TelegramService.sendMessage(
            `⚡ <b>kRISE/FALL2n IMMEDIATE RECOVERY</b>\n` +
    ...
    executeNextTrade(symbol, lastClosedCandle) {
        ...
        // ── CORE STRATEGY: Use assetMaxStreak for signal ──────────────
        const { currentStreak } = AlternatingRegimeDetector.analyze(assetState.closedCandles);
        const threshold = assetMaxStreak - 5;

        LOGGER.warn(
            `  [${symbol}] Streak Check: currentStreak=${currentStreak}, ` +
            `assetMaxStreak=${assetMaxStreak}, threshold=${threshold}`
        );
        ...
        if (currentStreak >= threshold) {
            ...
            if (candleType === 'BULLISH') {
                direction = 'CALLE';
                signalReason = `Streak Signal: currentStreak(${currentStreak}) >= threshold(${threshold}) after BULLISH candle`;
            } else if (candleType === 'BEARISH') {
                direction = 'PUTE';
                signalReason = `Streak Signal: currentStreak(${currentStreak}) >= threshold(${threshold}) after BEARISH candle`;
            } else {
                LOGGER.info(`  [${symbol}] DOJI candle — no trade`);
            }
            ...
            LOGGER.trade(`  [${symbol}] NORMAL MODE No Trade: currentStreak(${currentStreak}) < threshold(${threshold})`);
        }
    ...
    getStatus() {
        ...
        ACTIVE_ASSETS.forEach(symbol => {
            ...
            const ac = getAssetConfig(symbol);
            if (a) {
                const { currentStreak } = AlternatingRegimeDetector.analyze(a.closedCandles);
                ...
                assetStatuses[symbol] = {
                    ...
                    currentStreak,
                    assetMaxStreak,
                    threshold: assetMaxStreak ? assetMaxStreak - 5 : null,
    ...
    if (state.isAuthorized) {
        ...
        ACTIVE_ASSETS.forEach(sym => {
            ...
            if (a) {
                ...
                assetLines += `\n  ${sym} (${a.timeframe}/${a.duration}): M${a.martingaleLevel} $${a.currentStake.toFixed(2)} | ${a.trades}t ${a.wins}W/${a.losses}L | P/L:$${a.netPL.toFixed(2)} | Last:${dir}(${winLoss}) | Pos:${a.activePositions} | Streak:${a.currentStreak}/${a.assetMaxStreak}`;
            }
        });
