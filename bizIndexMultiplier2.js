...
/**
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║         DERIV SYNTHETIC INDICES MULTIPLIER BOT  —  v3.1                  ║
 * ║                                                                          ║
 ...
 * ║                                                                          ║
 * ║  LAYER 2 — TREND DIRECTION (2 of 3 must agree)                          ║
 ...
 * ║                                                                          ║
 * ║  ENTRY: All Layer 1 gates pass + Layer 2 agree (2/3) + Layer 3 score ≥ 3 ║
 * ║                                                                          ║
 * ║  MULTIPLIER SETTINGS:                                                    ║
 * ║    • CALLE signal → MULTUP (multiplier UP trade)                        ║
 * ║    • PUTE signal → MULTDOWN (multiplier DOWN trade)                     ║
 * ║    • Dynamic take_profit and stop_loss based on stake amount            ║
 * ║    • Stop Loss = current stake (max loss in multipliers)                ║
 * ║    • Take Profit = current stake × TAKE_PROFIT_MULTIPLIER (default 3x)  ║
 * ║    • Minimum multiplier per asset (typically x2-x5)                     ║
 * ║                                                                          ║
 * ║  RISK MANAGEMENT: Fixed-Fractional (1% of capital per trade)            ║
 * ║    • Conservative base: 1% per trade (replaces martingale)              ║
 * ║    • Optional limited recovery: max 3 steps, capped at 2.5% capital     ║
 * ║    • Hard daily stop-loss drawer protection                              ║
 * ║    • Three-consecutive-loss pause (cool-down before re-entry)            ║
 * ║                                                                          ║
 * ║  v3.1 CHANGES:                                                           ║
 * ║    ✔ Fixed ATR_THRESHOLDS for all assets (proportional to volatility)    ║
 * ║    ✔ Relaxed Layer 2: 2/3 trend agreement (was 3/3)                    ║
 * ║    ✔ Relaxed Layer 3: score ≥ 3 (was 4)                                  ║
 * ║    ✔ Fixed all syntax/HTML artifacts from previous paste                ║
 * ║    ✔ Re-enabled recovery stake cap for safety                          ║
 * ║    ✔ Added ATR to live status logging for real-time tuning              ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
 */
...
const CONFIG = {
    ...
    // Recovery staking (limited — max 3 steps)
    ...
    SESSION_PROFIT_TARGET:      50000,    // Stop after +$500 session gain
    SESSION_STOP_LOSS:          -15000,   // Stop after -$150 session loss
    DAILY_STOP_LOSS:            -20000,   // Hard daily stop, resets at UTC midnight

    // Cool-down after consecutive losses
    MAX_CONSECUTIVE_LOSSES:     7,      // Pause trading after 7 straight losses
    ...
    // FIXED v3.1: Reduced from 4 to 3 to allow more valid setups
...
class Indicators {
    ...
    static donchian(candles, period = 20) {
        ...
        return {
            ...
            lower:         low,
            mid:           (high + low) / 2,
            bullBreakout:  close > high,
            bearBreakout:  close < low,
        };
    }
...
class SignalAnalyzer {
    static analyze(candles, symbol) {
        ...
        if (score >= CONFIG.MIN_CONFLUENCE_SCORE) {
            result.direction  = trendDirection === 'BULL' ? 'MULTUP' : 'MULTDOWN';
...
class StakeCalculator {
    static calculate(capital, recoveryStep) {
        ...
        let multiplier = 1;
        for (let i = 0; i < recoveryStep; i++) {
            if (i === 0) multiplier *= CONFIG.RECOVERY_MULTIPLIER;
            else if (i === 1) multiplier *= CONFIG.RECOVERY_MULTIPLIER2;
            else multiplier *= CONFIG.RECOVERY_MULTIPLIER3;
        }
...
class TelegramService {
    ...
    static async sendTradeAlert(type, symbol, direction, stake, multiplier, details = {}) {
        ...
        const dynamicStopLoss = parseFloat(stake.toFixed(2));
    ...
    static async sendStartupMessage() {
        ...
        let pairInfo  = '';
        CONFIG.ACTIVE_ASSETS.forEach(sym => {
            const multiplier = CONFIG.MULTIPLIER_VALUES[sym] || 2;
        ...
        await this.sendMessage([
            ...
            `Take Profit: ${CONFIG.TAKE_PROFIT_MULTIPLIER}x stake | Stop Loss: 1x stake (dynamic based on stake)`,
...
class IndexBot {
    ...
    async start() {
        console.log('\n' + '═'.repeat(72));
        console.log(' DERIV SYNTHETIC INDICES MULTIPLIER BOT v3 — Volatility-Regime Adaptive');
        console.log('═'.repeat(72));
        console.log(`Assets    : ${CONFIG.ACTIVE_ASSETS.join(', ')}`);
        console.log(`Timeframe : ${CONFIG.TIMEFRAME_LABEL} candles | Market Type: MULTIPLIERS`);
        console.log(`Strategy  : L1(ATR+ADX+BB) + L2(ST+EMA+Donchian 2/3) + L3(RSI+MACD+Stoch+Pattern >=${CONFIG.MIN_CONFLUENCE_SCORE}/4)`);
        console.log(`Risk      : ${CONFIG.RISK_PERCENT_PER_TRADE}%/trade | Max: $${CONFIG.MAX_STAKE}`);
        console.log(`Multiplier: Take Profit: ${CONFIG.TAKE_PROFIT_MULTIPLIER}x stake | Stop Loss: 1x stake (dynamic based on stake)`);
        ...
        LOGGER.info('✅ Multiplier Bot v3 fully started!');
    }
    ...
    executeNextTrade(symbol, lastClosedCandle) {
        ...
        const multiplier = CONFIG.MULTIPLIER_VALUES[symbol] || 2;
        const dynamicStopLoss = parseFloat(stake.toFixed(2));
        const dynamicTakeProfit = parseFloat((stake * CONFIG.TAKE_PROFIT_MULTIPLIER).toFixed(2));
        const recoveryNote = a.recoveryStep > 0 ? `[RECOVERY STEP ${a.recoveryStep}]` : '';

        LOGGER.trade(
            `🎯 [${symbol}]${recoveryNote} ${signal.direction === 'MULTUP' ? '📈 MULTUP' : '📉 MULTDOWN'} |` +
            `Stake: $${stake.toFixed(2)} | Multiplier: x${multiplier} | Score: ${signal.score}/${signal.maxScore} | ADX: ${signal.indicators?.adx}`
        );
        LOGGER.trade(`   Take Profit: $${dynamicTakeProfit} | Stop Loss: $${dynamicStopLoss}`);
        ...
        CONFIG.ACTIVE_ASSETS.forEach(sym => {
            ...
            if (a) {
                ...
                pairStatuses[sym] = {
                    ...
                    cooldownCandles: a.cooldownCandles,
                    forceTradeOnLoss: a.forceTradeOnLoss,
...
console.log('\n🚀 Starting Deriv Multiplier Bot v3...\n');
...
const statusInterval = setInterval(() => {
    ...
    CONFIG.ACTIVE_ASSETS.forEach(sym => {
        ...
        if (p) {
            ...
            const atr    = p.indicators?.atr  ? `ATR:${p.indicators.atr}` : '';
            const cdwn   = p.cooldownCandles  > 0 ? `❄️CD:${p.cooldownCandles}` : '';
            const force  = p.forceTradeOnLoss ? `🔄FORCE:${p.lastDirection}` : '';
            pairLines += `\n  ${sym}: Rec${p.recoveryStep} $${p.currentStake.toFixed(2)} | ${p.trades}t ${p.wins}W/${p.losses}L $${p.netPL.toFixed(2)} | Pos:${p.activePositions}${cdwn}${force} | ${sig} | ${adx} ${rsi} ${atr}`;
        }
    });
