...
const CONFIG = Object.freeze({
  ...
  apiToken : 'pat_9407fd80466311ce13f2d2b5e884d95f6507f24a5dcb8ec853d775022c1591c7',
...
assets: envStr('ASSETS', 'BOOM500,BOOM600,BOOM900,BOOM1000,CRASH500,CRASH600,CRASH900,CRASH1000')
  ...
  breachMinCount : 75, // trade only when historical single-tick barrier hits ≥ this
  ...
  edgeMonitorMinTrades : 400000000000000,    // 40  minimum trades before edge monitor is active
  ...
  // ── Logging / state ──
  logFile   : envStr('LOG_FILE', 'accuFable5BC_02.log'),
  logLevel  : envStr('LOG_LEVEL', 'INFO'),
  stateFile : envStr('STATE_FILE', 'accuFable5BC-state_02.json'),
  edgeFile  : envStr('EDGE_FILE', 'accuFable5BC-edge_02.json'),
});
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        this.ws = new WebSocket(url, {
          headers: { 'User-Agent': 'AccuFable5BC/5.0 (+Node.js)' },
...
class RiskManager {
  ...
  halt(reason) {
    ...
    telegram.send(`⛔ <b>AccuFable5BC TRADING HALTED</b>\n${reason}`);
  }
...
class TradeExecutor extends EventEmitter {
  ...
  /**
   * TradeExecutor.openTrade: version matching accuFable5BC.
   * NOTE: body intentionally identical to accuFable5BC.js for parity.
   * All durations/timeout constants must stay in sync between the two bots.
   */
...
class AccuFable5 {
  constructor(cfg) {
    ...
    this.client.on('disconnected', () => {
      ...
      telegram.send(`⚠️ <b>AccuFable5BC Connection lost</b> — reconnecting\u2026`);
    });
...
logger.warn(`martingale MAX STEPS hit (${this.martingaleStep}/${maxSteps}) on loss — halting trading`);
  ...
  _sendTradeResult(t, rec, { prevStep, prevStake, mg }) {
    ...
    const msg =
      `${emoji} <b>AccuFable5BC TRADE ${label}</b>\n\n` +
  ...
  _sendEod(reason = 'manual') {
    ...
    let msg = `🌙 <b>AccuFable5BC DAILY REPORT — ${date}</b>\n\n`;
  ...
  async start() {
    ...
    telegram.send(
      `<b>AccuFable5BC Bot Online</b>${this.cfg.mode !== 'live' ? ' <b>🔒 DRY-RUN</b>' : ''}\n\n` +
...
function printBanner() {
  ...
  console.log(`
╭──────────────────────────────────────────────────────────────────────╮
│  AccuFable5 — measure-first Deriv Accumulator engine        │
├──────────────────────────────────────────────────────────────────────┤
│  mode          ${mode.padEnd(46)}│
...
async function main() {
  ...
  const bot = new AccuFable5(CONFIG);
...
module.exports = {
  ...
  TradeExecutor, MarketData, DerivClient, AccuFable5,
