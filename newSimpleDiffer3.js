...
/**
 * =====================================================================
 *  Deriv Digit Differ Trading Bot (SIMPLE edition)
 * =====================================================================
 *
 *  Single-file DIGITDIFF bot whose trading strategy is a straight
 *  transfer of accurateDiffer3_simple.js into this framework: coldest
 *  empirical-frequency digit with a Laplace-smoothed conservative
 *  P(loss) upper bound, recent-hit gate, per-digit prediction cooldown,
 *  and a live value-edge check against the DIGITDIFF payout.
 *
 *  ── Honesty note ──────────────────────────────────────────────
 ...
 *  no information about the next tick. The coldest-digit signal is a
 *  heuristic, not a demonstrated edge — it exists purely so the live
 *  value-edge check has *something* to compare against the payout.
 *  DIGITDIFF's payout already prices in the true ~90% hit rate plus
 *  house margin, so unselective play has negative expected value.
 *  The real work in this file is capital preservation: hard daily
 *  loss caps, a consecutive-loss circuit breaker, conservative
 *  fractional-Kelly sizing capped as a % of bankroll, and a
 *  per-symbol calibrator that sidelines a symbol the moment live
 *  results diverge from the model.
 *
 *  Decision stack (identical to accurateDiffer3_simple.js):
 *    • Barrier digit = coldest empirical-frequency digit, over one
 *      configurable lookback window (analysisWindow).
 *    • Laplace-smoothed P(loss): pEstimate = (count+1)/(n+10), capped
 *      at pUpper = min(0.15, pEstimate + 0.01).
 *    • Recent-hit gate + per-digit prediction cooldown (cooldownTicks).
 *    • Live proposal: q_be = 1 − ask/payout; require
 *          pUpper + safetyMargin ≤ q_be − minEdge
 ...
 *  Run:      node newSimpleDiffer.js
 *  Backtest: $env:BACKTEST=1; node newSimpleDiffer.js
 *
 * =====================================================================
 */
...
const CONFIG = Object.freeze({
  ...
  assets: ['RDBULL'], //'1HZ10V','1HZ25V','1HZ50V','1HZ75V','1HZ100V','R_10','R_25','R_50','R_75','R_100','RDBULL','RDBEAR'
  ...
  maxOpenTrades: 1,
  cooldownTicks: 300,        // don't re-analyze within N ticks of last trade
  // ── Asset rotation ────────────────────────────────────────────────
  ...
  //   (GMT/UTC). Set any day to false to disable trading on that
  //   day; open trades still settle normally.
  tradeSunday    : true,
  tradeMonday    : true,
  tradeTuesday   : true,
  tradeWednesday : true,
  ...
  // ── Analysis ──────────────────────────────────────────────────
  // Coldest-digit empirical-frequency analyzer (same as
  // accurateDiffer3_simple.js). Heuristic, not a claimed edge.
  analysisWindow:     100,       // digit frequency window
  minProbabilityGap:  0.005,     // min gap between coldest and 2nd coldest
  ...
  lossCooldownMs:     8000,      // MC loss cooldown after a loss
  ...
  // Persistence/logging
  stateFile: strEnv('STATE_FILE', 'simpleDiffer3_state_01.json'),
  logFile: strEnv('LOG_FILE', 'simpleDiffer3_bot_01.log'),
  ...
  // BACKTESTER
  // ═══════════════════════════════════════════════════════════════════
  //   Run with:  $env:BACKTEST=1; node newSimpleDiffer.js
  ...
  //
  //   BACKTEST_PAYOUT_MULT   (payout multiplier per 1 stake, default 1.10)
...
// ─────────────────────────────────────────────────────────────────────
// 7. SIMPLE COLDEST-DIGIT ANALYZER
//
//   The trading strategy is the same as accurateDiffer3_simple.js:
//   coldest digit over an empirical frequency window, Laplace-smoothed
//   P(loss) with a conservative cap, a recent-hit gate, a per-digit
//   prediction cooldown, and a value-edge check against the live
//   DIGITDIFF payout. Heuristic, not a claimed predictive edge.
// ─────────────────────────────────────────────────────────────────────
class SimpleDigitAnalyzer {
  constructor(cfg) {
    this.cfg = cfg;
    this._predictionLog = [];
   ...
   * This is a heuristic, not a predictive edge.
   */
  analyze(symbol, ticks) {
    ...
    let coldestDigit = 0;
    let coldestCount = counts[0];
    for (let d = 1; d < 10; d++) {
      if (counts[d] < coldestCount) {
        coldestDigit = d;
        coldestCount = counts[d];
      }
    }

    // Calculate probability (conservative upper bound)
    const n = recentDigits.length;
    const pEstimate = (coldestCount + 1) / (n + 10); // Laplace smoothing
    const pUpper = Math.min(0.15, pEstimate + 0.01); // Conservative cap
    ...
    for (let d = 0; d < 10; d++) {
      if (d !== coldestDigit && counts[d] < secondCount) {
        secondCount = counts[d];
      }
    }
    const gap = (secondCount - coldestCount) / n;
    if (gap < this.cfg.minProbabilityGap) {
      gates.push(`gap_low:${gap.toFixed(4)}`);
    }
...
class TradingBot {
  constructor() {
    ...
    this.analyzer = new SimpleDigitAnalyzer(this.cfg);
    ...
    // ── Monte Carlo state (from accurateDiffer3_simple.js) ─────────
    this.consecutiveMcSkips = 0;
    this.lastLossAt = 0;
    this.lastWeakSignalAt = 0;
  ...
  async _onAuthorized(info) {
    ...
    telegram.send(
      `🤖 <b>SIMPLE3 Digit Differ Bot Online</b>\n\n` +
  ...
  _onDisconnected(code, reason, wasAuthorized) {
    telegram.send(`⚠️ <b>SIMPLE3 Connection lost</b>\ncode: <code>${code}</code>\nwas authorized: ${wasAuthorized ? 'yes' : 'no'}\n🔄 reconnecting...`);
  ...
  async _analyzeAndTrade() {
    ...
    for (const a of mcApproved) {
      try {
        ...
        // Value edge: break-even loss prob minus pUpper minus safety margin
    ...
    if (!proposalCandidates.length) {
      logger.debug('no proposal candidates after payout probe');
      return;
    }
    ...
    if (!qualified.length) {
      ...
      if (top) {
        logger.info(`skip: best edge ${top.valueEdge.toFixed(4)} < minEdge ${this.cfg.minEdge} (${top.analysis.symbol} d${top.analysis.digit})`);
      }
      return;
    }
    ...
    if (best !== qualified[0]) {
      logger.info(
        `rotation: skipping locked ${qualified[0].analysis.symbol} → taking ${best.analysis.symbol} d${best.analysis.digit}`
      );
    }

    // ── Compute stake using Kelly + calibrator ────────────────────
    // Win prob = 1 - pUpper (conservative upper bound of loss prob)
    const pWin = 1 - best.analysis.pUpper;
  ...
  _onTradeOpen(t) {
    ...
    const a = t.analysis || {};
    telegram.send(
      `🟢 <b>TRADE OPENED — SIMPLE3 DIGIT DIFFER</b>\n\n` +
  ...
  _onTradeResult(t) {
    ...
    if (won0) {
      ...
      telegram.send(
        `🛑 <b>SIMPLE3 CIRCUIT BREAKER TRIPPED</b>\n\n` +
    ...
    // back to (1 − pUpper) if the field is missing (legacy trades).
    const won        = t.status === 'won';
    const pWinUsed   = Number(t.analysis?.predictedPWin
                        ?? (t.analysis?.pUpper != null ? 1 - Number(t.analysis.pUpper) : null));
    let calibNote = '';
    ...
    if (!won) {
      ...
      logger.info(`LOSS recorded — cooldown ${this.cfg.lossCooldownMs/1000}s active (consecutive skips: ${this.consecutiveMcSkips})`);
    } else {
      this.consecutiveMcSkips = 0;
    }

    // Analysis metadata for telegram
    const analysisMeta = t.analysis?.pUpper != null
      ? `\n📐 pUpper=${((t.analysis.pUpper)*100).toFixed(2)}% edge=${((t.analysis.valueEdge || 0)*100).toFixed(2)}pp`
      : '';
    ...
    telegram.send(
      `${emoji} <b>TRADE ${label} — SIMPLE3 DIGIT DIFFER</b>\n\n` +
      `🎫 Contract: <code>#${t.contractId}</code>\n` +
  ...
  async _recoverStuckTrade(reason) {
    ...
    telegram.send(
      `<b>SIMPLE3 STUCK TRADE RECOVERED [${reason}]</b>\n` +
      `Contract: ${contractId}\n` +
      `Asset: ${symbol}\n` +
  ...
  _sendHourly() {
    ...
    if (!list.length) {
      telegram.send(`⏰ <b>SIMPLE3 Hourly Summary GMT (${date} ${pad(hour)}:00-${pad(hour)}:59)</b>\n\nNo trades this hour.\n\n💼 Overall Profit: ${money(this.stats.overallProfit, this.currency())}`);
      return;
    }
    let msg = `⏰ <b>SIMPLE3 Hourly Summary GMT (${date} ${pad(hour)}:00-${pad(hour)}:59)</b>\n\n` +
  ...
  _sendEod(reason = 'manual') {
    ...
    let msg = `🌙 <b>SIMPLE3 END OF TRADE DAY — GMT</b>\n` +
    ...
    msg += `<b>──SIMPLE3 Overall / Stored Stats ──</b>\n` +
           `💼 Overall Profit: <b>${money(this.stats.overallProfit, this.currency())}</b>\n` +
    ...
    if (this.cfg.calibEnabled) {
      ...
      if (keys.length) {
        msg += `<b>──SIMPLE3 Symbol Calibration (rolling ${this.cfg.calibWindow}) ──</b>\n`;
  ...
  stop(signal) {
    ...
    telegram.send(`🛑 <b>SIMPLE3 Differ stopped</b>\nSignal: ${htmlEscape(signal)}\n💼 Overall Profit: ${money(this.stats.overallProfit, this.currency())}`);
    if (this._analysisT) clearInterval(this._analysisT);
...
// ─────────────────────────────────────────────────────────────────────
/**
 * Historical simulator for the coldest-digit DIGITDIFF strategy — the
 * same SimpleDigitAnalyzer the live bot uses.
 *
 ...
 *   4. Report: signals, wins, losses, empirical win-rate vs predicted
 *      P(win), edge distribution histogram, consecutive-loss streak
 *      statistics, calibration gap, and per-barrier-digit breakdowns.
 */
...
class DifferBacktester {
  ...
  async run(symbols) {
    ...
    console.log(
      `  analysisWindow=${this.cfg.analysisWindow}  ` +
      `minEdge=${this.cfg.minEdge}  safetyMargin=${this.cfg.safetyMargin}\n` +
      `           cooldownTicks=${this.cfg.cooldownTicks}  maxRecentHits=${this.cfg.maxRecentHits}  ` +
  ...
  _gateSnapshot() {
    return {
      ...
      maxRecentHits     : this.cfg.maxRecentHits,
      cooldownTicks     : this.cfg.cooldownTicks,
      durationTicks     : this.cfg.durationTicks,
  ...
  async _runOne(symbol) {
    ...
    const diag = {
      ...
      mcNotStable    : 0,
      mcEdgeLow      : 0,
      gatedEdge      : 0,   // value edge < minEdge
      gatedAssetLock : 0,
      recommended    : 0,
    ...
    while (i < ticks.length - duration - 1) {
      ...
      // Compute value edge using pUpper
      const ask = baseStake;
      ...
      let fire = true;
      if (valueEdge < this.cfg.minEdge)                        { fire = false; diag.gatedEdge++; }
      // Asset-lock
    ...
    console.log(`    gated (not allowed)  : ${diag.mcNotRecommended}`);
    ...
    if (diag.recommended === 0 && diag.scans > 0) {
      ...
      if (diag.bestEdgeSeen !== -Infinity && diag.bestEdgeSeen < this.cfg.minEdge) {
        ...
        console.log(`       Try lowering minEdge below ${(diag.bestEdgeSeen*100).toFixed(3)}%`);
      }
    }
    console.log(line + '\n');
...
function printBanner() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   Deriv Digit Differ Bot (SIMPLE edition)                  ║');
  console.log('║   DIGITDIFF • Coldest-digit • Value-Edge Gate             ║');
...
async function main() {
  ...
  if (process.env.BACKTEST === '1' || process.argv.includes('--backtest')) {
    ...
    console.log(`🧪 BACKTEST mode — coldest-digit strategy  symbols=[${list.join(', ')}]  ticks=${CONFIG.backtestTicks}`);
    console.log(`   analysisWindow=${CONFIG.analysisWindow}  minEdge=${CONFIG.minEdge}  safetyMargin=${CONFIG.safetyMargin}\n`);
