...
// FILE PATHS
// ══════════════════════════════════════════════════════════════════════════════
const STATE_FILE = path.join(__dirname, 'claudeWillb_01-state.json');
const HISTORY_FILE = path.join(__dirname, 'claudeWillb_01-history.json');
...
class BreakoutManager {
    ...
    static checkReversal(symbol) {
        ...
        if (assetState.activePosition?.pendingReversal || assetState.activePosition?.opening) return null;
...
class TelegramService {
    ...
    static async sendReversalAlert(symbol, direction, stake, previousLoss, reversalNumber, maxReversals, breakoutHigh, breakoutLow) {
        ...
        const message = [
            `\u{1f504} <b>REVERSALb TRADE #${reversalNumber}</b>`,
            `Asset: ${symbol}`,
    ...
    static async sendSessionSummary() {
        ...
        await this.sendMessage([
            `\u{1f4ca} <b>WILLb v7.4 SESSION SUMMARY</b>`,
    ...
    static async sendStartupMessage() {
        ...
        const overall = TradeHistoryManager.getOverallStats();
        await this.sendMessage([
            `\u{1f916} <b>WILLb BOT v7.4 STARTED (Multiplier)</b>`,
