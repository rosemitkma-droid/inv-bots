...
const CONFIG = Object.freeze({
  ...
  // assets: ('BOOM50,BOOM150N,BOOM300N,BOOM500,BOOM600,BOOM900,BOOM1000,CRASH50,CRASH150N,CRASH500,CRASH600,CRASH900,CRASH1000,CRASH1300N')
  //   .split(',').map(s => s.trim()).filter(Boolean),

  assets: ('BOOM1000').split(',').map(s => s.trim()).filter(Boolean),
  ...
  pulseGrowthRates    : [0.05], // eval multiple rates, let EV pick best
  ...
  apexPostSpikeMin        : parseInt('2', 10),
  apexPostSpikeWindowFrac : parseFloat('0.12'), // fraction of mean cadence
  apexMaxHazard           : parseFloat('0.002'), //0.002 reject if per-tick spike hazard > 1%
  ...
  apexBarrierSafety    : parseFloat('3.5'),

  // ─ Survival / EV requirements (per-class overrides of pulse* gates) ─
  apexMinSurvival     : parseFloat('0.922'),  // forward K-tick survival floor
  apexMinEV           : parseFloat('0.002'), //0.010 ≥ +1% net EV to fire
  apexMaxHoldBoom     : parseInt('5',  10),  // Boom/Crash hold cap (ticks)
  ...
  async _analyzeAndTrade() {
    try {
      ...
      logger.info(
        ...
        `sinceSpike=${best.ticksSinceSpike} v3:adjEdge=${(chosenCheck.adjustedEdge*100).toFixed(2)}%`,
      );
