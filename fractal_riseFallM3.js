...
class TelegramService {
    ...
    static async sendStartupMessage() {
        try {
            ...
            🕐 <b>Trading Sessions (GMT+1):</b>${sessionWindows}
        `.trim();
...
const CONFIG = {
    ...
    // ============================================
    // OPTIMIZED TRADING SESSION WINDOWS (ALL IN GMT+1)
    //
    // Research basis (Deriv Synthetic Indices — Fractal Breakout):
    //
    // SESSION 1 — ASIA OPEN (01:00–03:00 GMT+1 = 00:00–02:00 UTC)
    //   Clean early-session momentum as Asian markets open.
    //   Low noise; good for structure formation on R_50, R_75.
    //   V75 often shows key breakout moves around 00:00 UTC (23:00 GMT).
    //
    // SESSION 2 — LONDON OPEN (08:00–12:00 GMT+1 = 07:00–11:00 UTC)
    //   Best session for fractal breakouts on medium-volatility indices.
    //   R_50, R_75 show their cleanest trends here (Europe 07:00–13:00 UTC).
    //   1HZ indices are most active 08:00–12:00 GMT+2 = 07:00–11:00 UTC.
    //   Strongest directional structure; fractals confirmed quickly.
    //
    // SESSION 3 — NEW YORK OPEN (13:00–18:00 GMT+1 = 12:00–17:00 UTC)
    //   US Overlap peak for R_100, 1HZ50V (high-vol indices).
    //   Highest volume window (14:00–18:00 UTC); V100/1HZ move most here.
    //   Step indices (stpRNG) also trend well during NY hours.
    //   Extended to 18:00 GMT+1 to capture the full US Overlap benefit.
    //
    // SESSION 4 — LATE NIGHT (22:00–00:00 GMT+1 = 21:00–23:00 UTC)
    //   Second breakout window for V75 (key at 23:00 GMT).
    //   Cleaner signals as daily consolidation resolves.
    //   Good for catching the overnight continuation move.
    //
    // AVOIDED WINDOWS (based on research):
    //   03:00–08:00 GMT+1 (02:00–07:00 UTC): Dead zone — flat, wicks unpredictable.
    //   12:00–13:00 GMT+1: London–NY transition gap, false breakout risk.
    //   18:00–22:00 GMT+1: Post-NY wind-down, chop and consolidation.
...
class DerivBot {
    ...
    startSessionTimeChecker() {
        setInterval(() => {
            ...
            // Daily reconnection at 1:00 AM GMT+1 (start of ASIA session)
            if (
                !state.session.isActive &&
                currentHours === CONFIG.ASIA_START &&
                currentMinutes >= 0
            ) {
                LOGGER.info(
                    `It's ${CONFIG.ASIA_START}:00 AM GMT+1, reconnecting the bot and resetting daily session stats.`
                );
                state.session.isActive = true;
                this.connection.connect();
            }

            // End-of-day disconnect: after Late Night session ends,
            // only if ALL assets have recovered (no martingale in progress)
            if (state.session.isActive) {
                ...
                // Disconnect after Late Night session ends (00:00 GMT+1) + 30 min buffer
                // currentHours === 0 means we've crossed midnight, 30 min after LATENIGHT_END
                if (
                    allAssetsRecovered &&
                    anyAssetTradedWin &&
                    currentHours === 0 &&
                    ...
                    LOGGER.info(
                        `It's 00:30 GMT+1, all assets recovered, disconnecting for the dead zone.`
                    );
...
console.log('═'.repeat(80));
console.log(' DERIV RISE/FALL FRACTAL BREAKOUT BOT (Per-Asset Independent)');
console.log(` Base Stake: $${CONFIG.STAKE} | Optimized 4-Session Trading Windows`);
console.log('═'.repeat(80));
