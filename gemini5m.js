...
/**
 * DERIV.COM TRADING BOT - VOLATILITY 75 INDEX SCALPER
 * ====================================================
 * Market: Volatility 75 Index (R_75)
 * Strategy: EMA(9) / EMA(21) Crossover + RSI(14) Filter
 * Timeframe: 5 Minutes
 * 
 * INSTALLATION:
 * 1. npm install ws
...
${C.cyan}${C.bright}╔════════════════════════════════════════════════════════════╗
║                                                            ║
║     🤖 DERIV V75 EMA CROSSOVER + RSI SCALPING BOT          ║
...
╚════════════════════════════════════════════════════════════╝${C.reset}
    ...
    static displayConfig() {
        ...
        console.log(`  RSI Period:      ${C.bright}${CONFIG.RSI_PERIOD}${C.reset}`);
        console.log('');
    }
    ...
    static displayIndicators() {
        ...
        const { ema9, ema21, rsi } = STATE.indicators;
        const lastCandle = STATE.candles[STATE.candles.length - 1];
        const price = lastCandle ? lastCandle.close : '--';

        console.log(`  Price:       ${typeof price === 'number' ? price.toFixed(2) : price}`);
        console.log(`  EMA 9:       ${ema9 ? ema9.toFixed(2) : '--'}`);
        console.log(`  EMA 21:      ${ema21 ? ema21.toFixed(2) : '--'}`);
        console.log(`  RSI 14:      ${rsi ? rsi.toFixed(2) : '--'}`);

        if (ema9 && ema21) {
            const trend = ema9 > ema21 ? `${C.green}BULLISH${C.reset}` : `${C.red}BEARISH${C.reset}`;
    ...
    static displayStats() {
        ...
        const winRate = STATE.completedTrades.length > 0
            ? ((STATE.completedTrades.filter(t => t.finalPnL > 0).length / STATE.completedTrades.length) * 100).toFixed(1)
            : '0.0';
...
class Indicators {
    ...
    static calculateRSIArray(data, period = 14) {
...
class Strategy {
    static analyzeMarket(candles) {
        ...
        // Calculate EMAs
        const ema9Array = Indicators.calculateEMAArray(closes, CONFIG.EMA_FAST);
        const ema21Array = Indicators.calculateEMAArray(closes, CONFIG.EMA_SLOW);
        const rsiArray = Indicators.calculateRSIArray(closes, CONFIG.RSI_PERIOD);

        if (ema9Array.length < 2 || ema21Array.length < 2 || rsiArray.length < 1) {
            return { signal: 'WAIT', reason: 'Insufficient indicator data' };
        }

        // Current and previous values
        const currEma9 = ema9Array[ema9Array.length - 1];
        const prevEma9 = ema9Array[ema9Array.length - 2];
        const currEma21 = ema21Array[ema21Array.length - 1];
        ...
        const isCrossoverBuy = prevEma9 <= prevEma21 && currEma9 > currEma21;
        const isCrossoverSell = prevEma9 >= prevEma21 && currEma9 < currEma21;

        // Trend and RSI filters
        const isBullishTrend = currentPrice > currEma21;
        const isBearishTrend = currentPrice < currEma21;
        const isRsiBuy = currRSI > 50 && currRSI < CONFIG.RSI_OVERBOUGHT;
        const isRsiSell = currRSI < 50 && currRSI > CONFIG.RSI_OVERSOLD;

        // BUY Signal
        if (isCrossoverBuy && isBullishTrend && isRsiBuy) {
            return {
                signal: 'CALL',
                reason: 'EMA9 crossed above EMA21 + Bullish trend + RSI > 50',
                confidence: 'HIGH',
                indicators: { ema9: currEma9, ema21: currEma21, rsi: currRSI, price: currentPrice }
            };
        }

        // SELL Signal
        if (isCrossoverSell && isBearishTrend && isRsiSell) {
            return {
                signal: 'PUT',
                reason: 'EMA9 crossed below EMA21 + Bearish trend + RSI < 50',
                confidence: 'HIGH',
                indicators: { ema9: currEma9, ema21: currEma21, rsi: currRSI, price: currentPrice }
            };
        }
...
class DerivBot {
    ...
    async connect() {
        return new Promise((resolve, reject) => {
            ...
            // IMPORTANT: Include app_id in URL
    ...
    updateIndicators() {
        ...
        STATE.indicators.ema9 = Indicators.calculateEMA(closes, CONFIG.EMA_FAST);
        STATE.indicators.ema21 = Indicators.calculateEMA(closes, CONFIG.EMA_SLOW);
        ...
        if (STATE.currentCandleEpoch !== 0 && candleOpenTime > STATE.currentCandleEpoch) {
            ...
            if (lastCandle && lastCandle.time === STATE.currentCandleEpoch) {
                ...
                STATE.candles[STATE.candles.length - 1] = {
                    ...lastCandle,
                    close: candle.open // The new candle's open is the previous candle's close
                };
            }
    ...
    async analyzeAndTrade() {
        ...
        if (analysis.signal === 'CALL' || analysis.signal === 'PUT') {
            Logger.signal(`🎯 ${analysis.signal} SIGNAL DETECTED!`, analysis);
    ...
    handleContractUpdate(response) {
        ...
        if (contract.is_sold || contract.is_expired || contract.status === 'sold' || contract.status === 'won' || contract.status === 'lost') {
            ...
            if (trade.finalPnL > 0) {
                ...
                STATE.trading.consecutiveWins++;
                Logger.success(`🎉 Trade WON! Profit: +$${trade.finalPnL.toFixed(2)}`);
            } else {
                STATE.trading.consecutiveWins = 0;
