...
class EnhancedDerivTradingBot {
    ...
    analyzeTicksEnhanced() {
        ...
        this.lastDigit2 = this.tickHistory[this.tickHistory.length - 2];

        if(this.lastDigit === this.lastDigit2){
            console.log('Last two digits are the same, Trade is being Executed!.');
    ...
    async sendLossEmail() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'Grok Deriv 2 Deriv Bot - Loss Alert',
            text: summaryText
        };
    ...
    async sendErrorEmail(errorMessage) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'Grok Deriv 2 Deriv Bot - Error Report',
    ...
    async sendDisconnectResumptionEmailSummary() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'Grok Deriv 2 Deriv Bot - Status Update',
            text: summaryText
        };
