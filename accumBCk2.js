...
// FILE PATHS
// ============================================================
const STATE_FILE        = path.join(__dirname, 'accumulator_bot2-05_v1_state.json');
const HISTORY_FILE      = path.join(__dirname, 'accumulator_bot2-05_v1_history.json');
...
const CONFIG = {
    ...
    STAYED_IN_THRESHOLD:        7100,      // Asset active if total < this
    STAYED_IN_MAX_TOTAL:        7100,      // Max total sum for condition1
...
class TelegramService {
    ...
    static async sendTradeAlert(type, symbol, stake, profitDetails = {}) {
        ...
        const lines = [
            `${emoji} <b>ACCUMULATOR BOT2 v1 — ${type}</b>`,
            `Asset: <b>${symbol}</b>`,
...
class ConnectionManager {
    ...
    onClose() {
        ...
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
            ...
            TelegramService.sendMessage(`⚠️ <b>BOT2 CONNECTION LOST</b> — Reconnecting (attempt ${this.reconnectAttempts})`);
            setTimeout(() => { this.isReconnecting = false; this.connect(); }, delay);
        } else {
            LOGGER.error('Max reconnection attempts reached — giving up');
            TelegramService.sendMessage(`🛑 <b>BOT2 STOPPED</b> — Max reconnections\nFinal P/L: $${state.session.netPL.toFixed(2)}`);
            process.exit(1);
        }
    }
...
class AccumulatorBot {
    ...
    async start() {
        ...
        await TelegramService.sendMessage(
            `🤖 <b>ACCUMULATOR BOT2 v1.1 STARTED</b>\n` +
        ...
        setInterval(() => {
            ...
            if (lastWinAsset && !state.session.endedByTime) {
                if (hour >= CONFIG.DISCONNECT_HOUR && mins >= 0) {
                    ...
                    TelegramService.sendMessage(`🌙 <b>BOT2 Time Stop</b> — ${CONFIG.DISCONNECT_HOUR}:00 reached after win. Bot paused.`);
                    this.stop();
                }
            }
