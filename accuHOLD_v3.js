...
/**
 * =====================================================================
 *  accuHOLD v3 — tier-aware BOOM/CRASH ACCU bot (exploratory-wired)
 * =====================================================================
 *
 *  Single-file Deriv Accumulator (ACCU) bot — TEST/DEMO ONLY.
 *  Ported from accuHOLD_v2.js (memoryless) + hazardRefractoryBot_v2.js
 *  + deriv_structure_tester_v5.js. This build is EXPLORATORY-WIRED per
 *  user request: hazard + post-spike signals are FULLY WIRED to entry
 *  (CONFIG.exploratory.enabled:true, logOnly:false), even though
 *  rigorous testing found NO exploitable edge.
 *
 ...
 *  There is NO timing or price-pattern edge to trade on. This v3 build
 ...
 *
 *  ─ STRATEGY (v3 — tier-aware + exploratory) ─────────────────────
 ...
 *
 *  Author: Cowork 3P (v3 exploratory fork) | License: MIT
 * =====================================================================
 */
...
const CONFIG = Object.freeze({
  ...
  takeProfitMultiple : parseFloat('1.35'),   // fallback (tier overrides below)
  tickCapFraction    : parseFloat('0.55'),   // fallback (tier overrides below)
  growthRate         : parseFloat('0.02'),   // fallback default; tierGrowthRate below is authoritative

  // Per-tier overrides — user #5 (per-tier override) + #1 (include BOOM50/CRASH50 fast tier)
  tierGrowthRate: Object.freeze({ fast: 0.01, slow: 0.02 }), // fast=1% widest barrier (spike-dominant), slow=2% (noise-dominant)
  ...
  martingaleMultiplier : parseFloat('3.10'), // e.g. 3.10 means stake ×3.10 after each loss
  ...
  // ── Risk controls ──
  maxConsecutiveLosses : parseInt('7', 10),      // pause + require manual restart
...
class MarketDataManager extends EventEmitter {
  ...
  // Refresh per-tier growth buckets in one sweep (v3 tier-aware)
...
class AccuHoldBot {
  ...
  // ── Exploratory state ──────────────────────────────────
  _initExploratory() {
    this.assetHistory = new Map(); // symbol -> {prices:[], times:[], spikeIndices:[], intervals:[], hazardTable:null, disp:null, chi:null, ks:null, elevated:[], postStudy:null, ticksSinceSpike:0, meanInterval:null, pendingEntryAfter:null, pendingReason:null}
    const assets = (this.cfg && Array.isArray(this.cfg.assets) && this.cfg.assets.length) ? this.cfg.assets : CONFIG.assets;
    for (const s of assets) this.assetHistory.set(s, { prices:[], times:[], spikeIndices:[], intervals:[], hazardTable:null, disp:null, chi:null, ks:null, elevated:[], postStudy:null, ticksSinceSpike:0, meanInterval:null, pendingEntryAfter:null, pendingReason:null, lastSpikeAbsIdx:null, lastUpdate:0, calibrationStatus:'CALIBRATING'});
  ...
  _updateExploratoryForSymbol(symbol) {
    ...
    if (!h || h.prices.length < this.cfg.exploratory.baselineWindow + 100) return;
    // cap history
  ...
  async _onAuthorized(info) {
    ...
    if (!this._bootedOnce) {
      ...
      telegram.send(
        `<b>AccuHOLD v3 Bot Online</b>${this.dryRun ? ' <b>🔒 DRY-RUN</b>' : ''} <i>exploratory-wired</i>\n\n` +
      ...
      telegram.send(`🔄 <b>AccuHOLD_v3 Reconnected</b> (${info.loginid}, ${info.isVirtual ? 'DEMO' : 'REAL'})\n${martingaleLine.trim()}\n${exploratoryLine.trim()}${lossLine.trim()}`);
    }
  ...
  async _onDisconnected() {
    ...
    telegram.send(`⚠️ <b>AccuHOLD_v3 Connection lost</b> — reconnecting…`);
  ...
  _onTradeResult(t) {
    ...
    if (t.status === 'unknown') {
      ...
      telegram.send(`⚠️ <b>AccuHOLD_v3 UNCONFIRMABLE CONTRACT</b> #${t.contractId} ${t.symbol}\nRecorded as 'unknown' — excluded from P&L and streaks.`);
    ...
    const msg =
      `${emoji} <b>AccuHOLD_v3 TRADE ${label}</b>\n\n` +
  ...
  // ── Entry decision: EXPLORATORY-WIRED (hazard + postSpike + tier-aware) ─
  ...
  _sendEod(reason = 'manual') {
    ...
    let msg = `🌙 AccuHOLD_v3 <b>DAILY REPORT — ${date}</b>\n\n`;
  ...
  // ── State persistence (atomic write) — FRESH v3 (user #4) ──
  _saveState(reason = 'checkpoint') {
    ...
    try {
      const payload = {
        version: 3, engine: 'accuHOLD v3 (tier-aware, exploratory-wired)', savedAt: new Date().toISOString(), savedReason: reason,
  ...
  _loadState() {
    ...
    if (!fs.existsSync(file)) { logger.info('fresh state (v3) — no prior state file, starting clean per user #4'); return; }
    try {
      const d = JSON.parse(fs.readFileSync(file, 'utf8'));
      if (d.version && d.version < 3) { logger.info(`ignoring v${d.version} state (fresh v3 start) — archiving old state`); try{ fs.renameSync(file, file+'.v2bak'); }catch(_){} return; }
  ...
  stop(signal) {
    ...
    telegram.send(`<b>AccuHOLD_v3 Bot stopped</b>\nSignal: ${signal}`);
    if (this._analysisT) clearInterval(this._analysisT);
    if (this._proposalT) clearInterval(this._proposalT);
