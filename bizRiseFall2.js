...
const CONFIG = {
    ...
    RSI: {
        ENABLED: true,
        PERIOD: 14,
        OVERSOLD: 40,             // FIX #7: was 30, now more generous
    ...
    RISK: {
        ...
        COOLDOWN_MS: 30 * 60 * 1000, // FIX #5: 30-minute cooldown before resume
        DAILY_LOSS_LIMIT: -50,
...
// ============================================
// LOGGER UTILITY (moved AFTER CONFIG per FIX #3)
// ============================================
...
const LOGGER = {
    ...
    debug: msg => {
        // FIX #3: Guard against undefined CONFIG
        ...
        try {
            ...
            Object.keys(state.assets).forEach(symbol => {
                ...
                persistableState.assets[symbol] = {
                    ...
                    lastLossTime: asset.lastLossTime,
            ...
            if (savedData.assets) {
                Object.keys(savedData.assets).forEach(symbol => {
                    if (state.assets[symbol]) {
                        ...
                        asset.lastLossTime = saved.lastLossTime || 0;
...
class KellyPositionSizer {
     ...
     * FIX #5: Ensure stake doesn't drop below MIN_STAKE floor
     */
...
class SessionManager {
    ...
    /**
     * FIX #11: Record trade BEFORE checking day change
     */
    static recordTradeResult(symbol, profit, direction) {
        ...
        if (profit > 0) {
            ...
            assetState.lastLossTime = Date.now();  // FIX #5: Track last loss time
            ...
            if (assetState.consecutiveLosses >= CONFIG.RISK.MAX_CONSECUTIVE_LOSSES) {
                ...
                TelegramService.sendMessage(
                    ...
                    `Trading paused for this asset (30m cooldown).`
                );
            }
        }

        // FIX #11: Check day change AFTER recording trade
        this.checkDayChange();
    }
}
...
class ConnectionManager {
        ...
        CONFIG.ACTIVE_ASSETS.forEach(symbol => {
            if (!state.assets[symbol]) {
                state.assets[symbol] = {
                    ...
                    netPL: 0,
                    consecutiveLosses: 0,
                    lastLossTime: 0  // FIX #5: Track last loss time for cooldown
                };
    ...
    handleResponse(response) {
        if (response.msg_type === 'authorize') {
            ...
            // FIX #6: Always sync capital from Deriv's authoritative balance
            state.capital = response.authorize.balance;

            // Only set session startCapital on first connect or if not set
            if (state.session.startCapital === CONFIG.INITIAL_CAPITAL) {
    ...
    /**
     * FIX #9: Set currentFormingCandle to last loaded candle so first live OHLC
     * correctly detects a new candle close
     */
    handleOHLC(ohlc) {
    ...
    /**
     * FIX #9: Set currentFormingCandle to the LAST loaded candle
     * so the very next incoming OHLC tick will correctly detect a new candle
     */
    handleCandlesHistory(response) {
        ...
        // FIX #9: Set to last candle, not null, so next OHLC detects candle close
...
class DerivBot {
    ...
    async start() {
        console.log('\n' + '═'.repeat(80));
        console.log(' DERIV VOLATILITY BREAKOUT BOT - PROFESSIONAL STRATEGY (FIXED)');
        ...
        console.log(`  Breakout MIN    : ${CONFIG.BREAKOUT.MIN_STRENGTH * 100}% (FIXED from 120%)`);
        console.log('═'.repeat(80) + '\n');
    ...
    /**
     * FIX #8: Move canTrade = false to TOP of function to prevent race conditions
     * FIX #4: Seed Kelly with historical win rate
     * FIX #5: Check for cooldown and auto-resume paused assets
     * FIX #10: Update currentStake after calculation
     */
    executeNextTrade(symbol, lastClosedCandle) {
        const assetState = state.assets[symbol];
        if (!assetState) return;

        // FIX #8: Lock IMMEDIATELY to prevent race conditions with multiple candle closes
        ...
        if (assetState.consecutiveLosses >= CONFIG.RISK.MAX_CONSECUTIVE_LOSSES) {
            ...
            const cooldownExpired = (Date.now() - lastLossTime) >= CONFIG.RISK.COOLDOWN_MS;

            if (!cooldownExpired) {
                const minutesLeft = Math.round((CONFIG.RISK.COOLDOWN_MS - (Date.now() - lastLossTime)) / 60000);
                LOGGER.debug(`${symbol} in cooldown (${minutesLeft}m left)`);
                return;
            }
        ...
        // FIX #4: Seed Kelly with historical win rate if available
        const overall = TradeHistoryManager.getOverallStats();
        ...
        LOGGER.trade(`  Win Rate (hist.): ${(historicalWinRate * 100).toFixed(1)}%`);
...
console.log(' ✓ Capital synced from account balance');
console.log(' ✓ Race condition fixed (canTrade lock first)');
console.log(' ✓ Candle warmup fixed (currentFormingCandle)');
console.log(' ✓ All 12 bugs fixed');
console.log('═'.repeat(80));
console.log('\n🚀 Initializing...\n');
