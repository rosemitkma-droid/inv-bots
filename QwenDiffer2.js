...
class EnhancedDigitDifferTradingBot {
    constructor(token, config = {}) {
        ...
        this.config = {
            ...
            volatilityLookback: config.volatilityLookback || 12,   // ticks to check for breakout
            minUniqueDigits: config.minUniqueDigits || 7,          // trigger if ≥7 unique digits in lookback
            clusterLookback: config.clusterLookback || 3,          // ticks to check for compression (e.g., 4,5,4)
        ...
        this.rStats = {};
        this.sys = 1;
        this.engineMode = 'BREAKOUT'; // 'BREAKOUT' or 'REVERSION_FALLBACK'
    ...
    handleTickUpdate(tick) {
        ...
        console.log(`[${asset}] ${tick.quote} → Last 5 Digits: ${this.tickHistories[asset].slice(-5).join(', ')}`);
    ...
    // ========= 🎯 CORE LOGIC: VOLATILITY BREAKOUT + SYNTHETIC MOMENTUM =========
    analyzeTicks(asset) {
        ...
        if (!isHighVolatility) {
            console.log(`🔇 [${asset}] Low volatility burst (${uniqueDigits}/${this.config.minUniqueDigits} unique digits). Skipping.`);
            return;
        }

        // ▼▼▼ CLUSTER DETECTION: Check if recent digits are compressed (e.g., 4,5,4) ▼▼▼
        const isClustered = this.isCompressedCluster(recentClusterWindow);
        let predictedDigit;

        if (isClustered) {
            // ▼▼▼ BREAKOUT MODE: Predict outlier digit not in cluster range ▼▼▼
            const clusterMin = Math.min(...recentClusterWindow);
            const clusterMax = Math.max(...recentClusterWindow);
            const possibleOutliers = [];
            for (let d = 0; d <= 9; d++) {
                if (d < clusterMin - 1 || d > clusterMax + 1) {
                    possibleOutliers.push(d);
                }
            }

            if (possibleOutliers.length !== 0) {
                predictedDigit = possibleOutliers[crypto.randomInt(0, possibleOutliers.length)];
                console.log(`🧨 [${asset}] Cluster detected [${recentClusterWindow.join(',')}] → Predicting OUTLIER: ${predictedDigit}`);
                this.placeTrade(asset, predictedDigit, { vol: uniqueDigits, clustered: isClustered });
            } else {

                console.log(`🧨 [${asset}] Cluster detected [${possibleOutliers.join(',')}]`);
            }
        }
    }

    isCompressedCluster(digits) {
        if (digits.length < 2) return false;
        const range = Math.max(...digits) - Math.min(...digits);
        return range <= 2; // e.g., [4,5,4] → range=1 → compressed
    }

    getLeastRecentDigit(history) {
        const lastSeenIndex = Array(10).fill(-1);
        for (let i = 0; i < history.length; i++) {
            lastSeenIndex[history[i]] = i;
        }
        let leastRecentDigit = 0;
        let oldestIndex = Infinity;
        for (let d = 0; d < 10; d++) {
            if (lastSeenIndex[d] < oldestIndex) {
                oldestIndex = lastSeenIndex[d];
                leastRecentDigit = d;
            }
        }
        return leastRecentDigit;
    }
    ...
    placeTrade(asset, predictedDigit, metadata) {
        if (this.tradeInProgress) return;

        this.tradeInProgress = true;
        this.xDigit = predictedDigit;

        console.log(`🚀 [${asset}] Placing trade → Digit: ${predictedDigit} | Stake: $${this.currentStake} | Vol: ${metadata.vol} | Clustered: ${metadata.clustered}`);
    ...
    handleTradeResult(contract) {
        ...
        if (won) {
            ...
            this.consecutiveLosses = 0;
            this.currentStake = this.config.initialStake;
            // this.engineMode = 'BREAKOUT'; // reset to primary mode
        } else {
            this.totalLosses++;
            this.consecutiveLosses++;
            this.isWinTrade = false;

            // ▼▼▼ SWITCH TO FALLBACK ENGINE AFTER LOSS ▼▼▼
            if (this.engineMode === 'BREAKOUT') {
                this.engineMode = 'REVERSION_FALLBACK';
            } else {
                this.engineMode = 'BREAKOUT';
            }
    ...
    async sendEmailSummary() {
        ...
        const mailOptions = {
            ...
            subject: 'QwenDigit_Differ2-Multi_Asset_Bot - Summary',
            text: summaryText
        };
    ...
    async sendLossEmail(asset) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'QwenDigit_Differ2-Multi_Asset_Bot - Loss Alert',
            text: summaryText
        };
    ...
    async sendDisconnectResumptionEmailSummary() {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'QwenDigit_Differ2-Multi_Asset_Bot - Connection/Dissconnection Summary',
            text: summaryText
        };
    ...
    async sendErrorEmail(errorMessage) {
        ...
        const mailOptions = {
            from: this.emailConfig.auth.user,
            to: this.emailRecipient,
            subject: 'QwenDigit_Differ2-Multi_Asset_Bot - Error Report',
