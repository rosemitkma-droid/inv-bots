 ...
 * ║        → MULTDOWN, mark prev candle High/Low as breakout levels         ║
 * ║  REVERSAL: BUY→SELL when candle CLOSES BELOW lower, SELL→BUY when       ║
 * ║           CLOSES ABOVE higher — each reversal is loss/win with          ║
 * ║           recovery multiplier (modified: TP = initTP + cumulativeLoss)   ║
 * ║  Win partial TP: remainder = takeProfitAmount - profit added to next      ║
 * ║            stake+TP (both). Loss reversal: cumulativeLoss += |loss|.    ║
 * ║  Persistent levels: stay active until opposite type formed; after TP     ║
 * ║  wait for price between levels, new trades when close >high / <low.    ║
 * ╚══════════════════════════════════════════════════════════════════════════╝
 */
...
// ============================================================
const STATE_FILE = path.join(__dirname, 'bizWillMulRF_v2_04-state.json');
const HISTORY_FILE = path.join(__dirname, 'bizWillMulRF_v2_04-history.json');
...
// ============================================================
// TIMEFRAME CONFIGURATION — 15m default, user can set to 1m/5m/15m etc.
// ============================================================
const TIMEFRAMES = {
...
const CONFIG = {
    ...
    // ── Recovery Strategy (from WILLMULT_v2.js) ──────────────
    ...
    // NOTE: DURATION removed for multipliers — positions stay open until TP/SL or opposite WPR/breakout
...
// ============================================================
// PER-ASSET CONFIGURATION (from WILLMULT_v2.js)
// ============================================================
const DEFAULT_ASSET_CONFIG = {
    ...
    CONTINUE_EXTRA_LEVELS: 9, // Number of extra levels to continue after max martingale level reached
...
// ============================================================
// BREAKOUT MANAGER — Persistent breakout levels (from cluadeWill.js)
// ============================================================
class BreakoutManager {
    static setupBreakoutLevels(symbol, direction, breakoutType) {
        const a = state.assets[symbol];
        const closed = a.closedCandles;
        if (closed.length < 1) { LOGGER.warn(`${symbol}: Not enough candles for breakout`); return false; }
        const prevCandle = closed[closed.length - 1];
        const highLevel = Number(prevCandle.high);
        const lowLevel = Number(prevCandle.low);
    ...
    static replaceBreakoutLevels(symbol, newType) {
        const a = state.assets[symbol];
        const closed = a.closedCandles;
        ...
        LOGGER.breakout(`${symbol} REPLACING ${a.breakout.type} → ${newType}`);
        a.breakout = { active: true, type: newType, highLevel, lowLevel, triggerCandle: prevCandle.open_time ?? prevCandle.epoch, canBeReplaced: false };
    ...
    static checkReversal(symbol) {
        const a = state.assets[symbol];
        const breakout = a.breakout;
        const closed = a.closedCandles;
        if (!a.inTradeCycle || !breakout?.active || closed.length < 1) return null;
        if (a.activePositions?.[0]?.sellActive) return null;
        ...
        const curDir = a.currentDirection || a.activePositions?.[0]?.direction;
        if (![lastClose, highLevel, lowLevel].every(Number.isFinite)) return null;
        // MULTUP/BULL = UP, MULTDOWN/BEAR = DOWN
        const isUp = curDir === 'UP' || curDir === 'MULTUP' || curDir === 'CALLE';
        const isDown = curDir === 'DOWN' || curDir === 'MULTDOWN' || curDir === 'PUTE';
        if (isUp && lastClose < lowLevel) {
            LOGGER.breakout(`${symbol} REVERSAL BUY→SELL close ${lastClose.toFixed(5)} < low ${lowLevel.toFixed(5)}`);
            return 'DOWN';
        }
        if (isDown && lastClose > highLevel) {
            LOGGER.breakout(`${symbol} REVERSAL SELL→BUY close ${lastClose.toFixed(5)} > high ${highLevel.toFixed(5)}`);
            return 'UP';
        }
        return null;
    }
    ...
    static checkForBreakoutReplacement(symbol) {
        const a = state.assets[symbol];
        const wpr = a.wpr, prevWpr = a.prevWpr, breakout = a.breakout;
        if (!breakout?.active || !a.inTradeCycle || !Number.isFinite(wpr) || !Number.isFinite(prevWpr)) return null;
        if (a.activePositions?.[0]?.sellActive) return null;
        if (breakout.type === 'BUY') {
            const isCrossBelow = prevWpr >= CONFIG.WPR_OVERSOLD && wpr < CONFIG.WPR_OVERSOLD;
            if (isCrossBelow && a.sellFlagActive) {
                LOGGER.signal(`${symbol} NEW SELL BREAKOUT during BUY cycle`);
                if (this.replaceBreakoutLevels(symbol, 'SELL')) { a.sellFlagActive = false; if (a.currentDirection === 'UP' || a.currentDirection === 'MULTUP' || a.currentDirection === 'CALLE') return 'DOWN'; }
            }
        }
        if (breakout.type === 'SELL') {
            const isCrossAbove = prevWpr <= CONFIG.WPR_OVERBOUGHT && wpr > CONFIG.WPR_OVERBOUGHT;
            if (isCrossAbove && a.buyFlagActive) {
                LOGGER.signal(`${symbol} NEW BUY BREAKOUT during SELL cycle`);
                if (this.replaceBreakoutLevels(symbol, 'BUY')) { a.buyFlagActive = false; if (a.currentDirection === 'DOWN' || a.currentDirection === 'MULTDOWN' || a.currentDirection === 'PUTE') return 'UP'; }
            }
        }
        return null;
    }
    ...
    static setWaitingForReentry(symbol, completedDirection = null) {
        ...
        a.waitingForReentry = true;
        if (completedDirection) a.lastTradeDirection = completedDirection;
        a.breakout.canBeReplaced = true;
        LOGGER.breakout(`${symbol} TP REACHED — waiting for re-entry, levels High ${a.breakout.highLevel.toFixed(5)} Low ${a.breakout.lowLevel.toFixed(5)}`);
    }

    static clearBreakout(symbol) {
        const a = state.assets[symbol];
        LOGGER.breakout(`${symbol} BREAKOUT CLEARED`);
        a.breakout = { active: false, type: null, highLevel: 0, lowLevel: 0, triggerCandle: 0, canBeReplaced: true };
        a.inTradeCycle = false;
        a.waitingForReentry = false;
        a.priceReturnedToZone = false;
    }
...
// ============================================================
// SIGNAL MANAGER — WPR + Breakout integrated (15m, WPR7)
// ============================================================
class SignalManager {
    static seedWPRState(symbol) {
        ...
        const period = cfg.WPR_PERIOD ?? CONFIG.WPR_PERIOD ?? 7;
        const series = TechnicalIndicators.calculateWPRSeries(a.closedCandles, period);
        if (!series.length) return false;
        let buyArmed = false, sellArmed = false, previous = null;
        for (const point of series) {
            const cur = point.value;
            if (cur <= CONFIG.WPR_OVERSOLD) buyArmed = true;
            if (cur >= CONFIG.WPR_OVERBOUGHT) sellArmed = true;
            if (Number.isFinite(previous)) {
                if (previous <= CONFIG.WPR_OVERBOUGHT && cur > CONFIG.WPR_OVERBOUGHT && buyArmed) buyArmed = false;
                if (previous >= CONFIG.WPR_OVERSOLD && cur < CONFIG.WPR_OVERSOLD && sellArmed) sellArmed = false;
            }
            previous = cur;
        }
    ...
    static updateWPRState(symbol) {
        ...
        if (a.wpr <= CONFIG.WPR_OVERSOLD && !a.buyFlagActive) { a.buyFlagActive = true; LOGGER.wpr(`${symbol}: BUY FLAG ARMED — WPR entered oversold (${a.wpr.toFixed(2)})`); }
        if (a.wpr >= CONFIG.WPR_OVERBOUGHT && !a.sellFlagActive) { a.sellFlagActive = true; LOGGER.wpr(`${symbol}: SELL FLAG ARMED — WPR entered overbought (${a.wpr.toFixed(2)})`); }
        // If inTradeCycle + waitingForReentry, block WPR signals — only price-action re-entry allowed
        if (a.inTradeCycle) return false;
        if (a.waitingForReentry && a.breakout?.active) {
            LOGGER.debug(`${symbol}: WPR blocked — waiting for price re-entry between levels`);
            return false;
        }
        return false;
    }

    static _snapshot(a) { return { breakout: { ...a.breakout }, inTradeCycle: a.inTradeCycle, waitingForReentry: a.waitingForReentry, priceReturnedToZone: a.priceReturnedToZone }; }
    static _restore(a, snap) { a.breakout = snap.breakout; a.inTradeCycle = snap.inTradeCycle; a.waitingForReentry = snap.waitingForReentry; a.priceReturnedToZone = snap.priceReturnedToZone; }

    static checkBuySignal(symbol) {
        const a = state.assets[symbol];
        const wpr = a?.wpr, prevWpr = a?.prevWpr;
        if (!Number.isFinite(wpr) || !Number.isFinite(prevWpr)) return { shouldTrade:false, reason:`WPR not ready (${String(prevWpr)}→${String(wpr)})`, details:{wpr,prevWpr} };
        const isCrossAbove = prevWpr <= CONFIG.WPR_OVERBOUGHT && wpr > CONFIG.WPR_OVERBOUGHT;
    ...
    static checkSellSignal(symbol) {
        const a = state.assets[symbol];
        const wpr = a?.wpr, prevWpr = a?.prevWpr;
        if (!Number.isFinite(wpr) || !Number.isFinite(prevWpr)) return { shouldTrade:false, reason:`WPR not ready`, details:{wpr,prevWpr} };
        const isCrossBelow = prevWpr >= CONFIG.WPR_OVERSOLD && wpr < CONFIG.WPR_OVERSOLD;
    ...
    static checkReentrySignal(symbol) {
        ...
        const breakout = a.breakout;
        const closed = a.closedCandles;
        if (!breakout?.active || closed.length < 1) return null;
        const closePrice = Number(closed[closed.length - 1].close);
        const isBetween = closePrice > breakout.lowLevel && closePrice < breakout.highLevel;
        if (isBetween) {
            if (!a.priceReturnedToZone) { a.priceReturnedToZone = true; LOGGER.signal(`${symbol} PRICE RETURNED TO ZONE (${closePrice.toFixed(5)}) Zone ${breakout.lowLevel.toFixed(5)} - ${breakout.highLevel.toFixed(5)}`); }
            return null;
        }
        if (!a.priceReturnedToZone) return null;
        if (closePrice > breakout.highLevel) { LOGGER.signal(`${symbol} RE-ENTRY BUY: ${closePrice.toFixed(5)} > ${breakout.highLevel.toFixed(5)}`); return 'UP'; }
        if (closePrice < breakout.lowLevel) { LOGGER.signal(`${symbol} RE-ENTRY SELL: ${closePrice.toFixed(5)} < ${breakout.lowLevel.toFixed(5)}`); return 'DOWN'; }
        return null;
    }
    ...
    static analyze(symbol) {
        ...
        const a = state.assets[symbol];
        return { shouldTrade:false, direction:null, confidence:0, reason: buySig.reason || sellSig.reason || `No WPR wpr=${a?.wpr?.toFixed(2) ?? 'n/a'}`, details:{wpr:a?.wpr, prevWpr:a?.prevWpr} };
    }
}
        ...
        try {
            ...
            Object.keys(state.assets).forEach(symbol => {
                ...
                data.assets[symbol] = {
                    ...
                    breakout: a.breakout ? { ...a.breakout } : { active: false, type: null, highLevel: 0, lowLevel: 0, triggerCandle: 0, canBeReplaced: true },
                    ...
                    reversalLevel: a.reversalLevel || 0,
                    accumulatedLoss: a.accumulatedLoss || a.cumulativeLoss || 0,
                    takeProfit: a.takeProfit || parseFloat((getAssetConfig(symbol).INITIAL_STAKE * CONFIG.TAKE_PROFIT_MULTIPLIER).toFixed(2)),
                    takeProfitAmount: a.takeProfitAmount || a.cumulativeLoss || 0,
                    // Normal mode state
                    ...
                    pendingReversal: a.pendingReversal || null,
                    // Stats
            ...
            if (data.assets) {
                Object.keys(data.assets).forEach(symbol => {
                    if (state.assets[symbol]) {
                        ...
                        a.accumulatedLoss = Number(saved.accumulatedLoss ?? saved.cumulativeLoss) || 0;
                        a.cumulativeLoss = a.accumulatedLoss; // keep alias
                        a.takeProfit = Number(saved.takeProfit) || parseFloat((getAssetConfig(symbol).INITIAL_STAKE * CONFIG.TAKE_PROFIT_MULTIPLIER).toFixed(2));
                        a.takeProfitAmount = Number(saved.takeProfitAmount) || a.cumulativeLoss || 0;
                        ...
                        a.pendingReversal = saved.pendingReversal || null;
...
class TelegramService {
    ...
    static async sendTradeAlert(type, symbol, direction, stake, duration, durationUnit, details = {}) {
        ...
        // Build analysis details for OPEN trades — supports both Rise/Fall and Multipliers
        let analysisDetails = '';
        if (type === 'OPEN' && details) {
            if (details.isRecovery) {
                if (details.multiplier) {
                    analysisDetails = `
        🔄 <b>RECOVERY MODE: YES</b> Multiplier x${details.multiplier} — TP $${details.takeProfit} SL $${details.stopLoss}
        ⚡ Same direction as loss trade (exclusive until win)`;
                } else {
                    analysisDetails = `
        ...
        ⚡ Same direction as loss trade (NO pattern analysis)`;
                ...
                const multInfo = details.multiplier ? ` | Multiplier x${details.multiplier} TP $${details.takeProfit} SL $${details.stopLoss}` : '';
                analysisDetails = `
        🧠 <b>WPR(${CONFIG.WPR_PERIOD}) Signal:</b>
        📊 WPR: ${wprStr} (OB ${CONFIG.WPR_OVERBOUGHT} / OS ${CONFIG.WPR_OVERSOLD})${multInfo}
        📊 Signal: ${analysis?.direction || 'N/A'} (${analysis?.reason || ''})`;
            } else if (details.multiplier) {
                analysisDetails = `
        ...
        const recoveryStatus = a?.isRecovery ? '🔄 RECOVERY' : (a?.waitingForNewSignal ? '⏳ WAIT' : '🎯 NORMAL');
        const isMultiplier = direction === 'MULTUP' || direction === 'MULTDOWN' || details?.multiplier;
        const dirLabel = direction === 'CALLE' || direction === 'MULTUP' ? 'RISE 📈' : direction === 'PUTE' || direction === 'MULTDOWN' ? 'FALL 📉' : direction;
        const stakeLine = `💵 Stake: $${stake.toFixed(2)}` + (details?.multiplier ? ` | Multiplier x${details.multiplier} | TP $${details.takeProfit ?? '?'} SL $${details.stopLoss ?? '?'}` : '');
        const durationLine = isMultiplier ? '' : `⏱ Duration: ${duration}${(durationUnit || 's').toUpperCase()}\n        `;

        const msg = `
        ${emoji} <b>${type} WILLMULT_v2 TRADE ALERT - ${recoveryStatus}</b>

        📊 Asset: ${symbol}
        📈 Direction: ${dirLabel} (${direction})
        ${stakeLine}
        ${durationLine}🔢 Martingale Level: ${a ? a.martingaleLevel : 0}
    ...
    static async sendHourlySummary() {
        ...
        await this.sendMessage([
            `⏰ <b>WILLMULT_v2 BOT HOURLY SUMMARY</b>`,
    ...
    static async sendSessionSummary() {
        ...
        await this.sendMessage([
            `\u{1f4ca} <b>WILLMULT_v2 BOT SESSION SUMMARY</b>`,
    ...
    static async sendStartupMessage() {
        ...
        let pairInfo = '';
        CONFIG.ACTIVE_ASSETS.forEach(sym => {
            const mult = CONFIG.MULTIPLIER_VALUES[sym] || 200;
            pairInfo += `\n  ${sym}: ${CONFIG.TIMEFRAME_LABEL} | x${mult} | TP ${CONFIG.TAKE_PROFIT_MULTIPLIER}x SL 1x`;
        });

        await this.sendMessage([
            `\u{1f916} <b>WILLMULT_v2 MULTIPLIER BOT STARTED</b>`,
            ...
            `<b>Active Assets (Multiplier):</b>${pairInfo}`,
        ].join('\n'));
    }
...
class SessionManager {
    ...
    static checkDayChange() {
        ...
        if (state.currentTradeDay && state.currentTradeDay !== today) {
            ...
            TelegramService.sendMessage(
                `\u{1f319} <b>WILLMULT_v2 BOT END OF DAY ${state.currentTradeDay}</b>\nP/L: $${(dayStats?.netPL || 0).toFixed(2)}\nCapital: $${state.capital.toFixed(2)}`
            );
    ...
    static recordTradeResult(symbol, profit, direction, stake) {
        ...
        if (profit > 0) {
            ...
            // ── Breakout v2 win handling: check if TP was fully met vs partial win via opposite-signal close ──
            const tpTarget = Number(a.takeProfitAmount) || 0;
            const isPartialWin = tpTarget > 0 && profit < tpTarget - 0.001;
            if (isPartialWin) {
                const remainder = parseFloat((tpTarget - profit).toFixed(2));
                // Carry remainder to next trade: both stake and TP increase by remainder
                a.accumulatedLoss = remainder;
                a.cumulativeLoss = remainder;
                a.takeProfitAmount = parseFloat((a.takeProfit + remainder).toFixed(2));
                a.consecutiveLosses = 0; // not a loss, but keep exclusive until full TP?
                a.martingaleLevel = Math.max(0, a.martingaleLevel); // keep level, stake will be remainder-based
                a.currentStake = StakeCalculator.calculate(symbol, a.martingaleLevel, a.investmentRemaining, a.cumulativeLoss);
                // For breakout, keep levels active and set waiting for re-entry? No — partial win still in trade cycle, will reverse?
                // Keep inTradeCycle true, breakout active, but update stake/TP for next reversal
                LOGGER.trade(`WIN (partial) [${symbol}] +$${profit.toFixed(2)} < TP $${tpTarget.toFixed(2)} — remainder $${remainder.toFixed(2)} added to next stake/TP (now $${a.currentStake.toFixed(2)} TP $${a.takeProfitAmount.toFixed(2)})`);
                // Do not clear exclusiveLock/inTradeCycle for partial — will be handled via reversal or next breakout
                ...
                // Keep breakout active, but if TP not met via opposite close, next reversal will use updated stake
            } else {
                // Full TP win
                ...
                a.accumulatedLoss = 0;
                a.reversalLevel = 0;
                a.takeProfitAmount = a.takeProfit;
                a.cooldownCandles = 0;
                a.lastTradeWasWin = true;
                a.forceRecoverDirection = null;

                // Exit all recovery/exclusive locks on full win — but keep breakout persistent levels for re-entry
                ...
                if (wasRecovery || wasWaiting || wasLocked) {
                    ...
                    if (wasRecovery) LOGGER.info(`[${symbol}] Recovery mode EXITED - Full TP Win → revert to ${getAssetConfig(symbol).DURATION}${getAssetConfig(symbol).DURATION_UNIT}`);
                    if (wasWaiting) LOGGER.info(`[${symbol}] Waiting-for-signal mode EXITED - Full TP Win`);
                    if (wasLocked) LOGGER.info(`[${symbol}] Exclusive lock RELEASED - Full TP Win`);
                }
                a.pendingRecovery = false;
                a.recoveryFirstDone = false;
                // For breakout persistent levels: after TP, keep breakout active but set waitingForReentry
                if (a.breakout?.active) {
                    BreakoutManager.setWaitingForReentry(symbol, direction);
                } else {
                    a.inTradeCycle = false;
                    a.waitingForReentry = false;
                }
                ...
                LOGGER.trade(`WIN [${symbol}] +$${profit.toFixed(2)} | ${direction} | P/L: $${(a.netPL || 0).toFixed(2)}`);
            }
        } else {
            ...
            a.reversalLevel = (a.reversalLevel || 0) + 1;
            ...
            a.accumulatedLoss = a.cumulativeLoss;
            a.takeProfitAmount = parseFloat((a.takeProfit + a.accumulatedLoss).toFixed(2));
            ...
            if (a.consecutiveLosses >= CONFIG.MAX_CONSECUTIVE_LOSSES) {
                ...
                TelegramService.sendMessage(
                    `❄️ <b>[${symbol}] WILLMULT_v2 WPR BOT COOL-DOWN ACTIVATED</b>\n` +
...
class ConnectionManager {
        ...
        CONFIG.ACTIVE_ASSETS.forEach(symbol => {
            if (!state.assets[symbol]) {
                ...
                state.assets[symbol] = {
                    ...
                    // Reversal stake state (v2) — modified recovery + win-remainder
                    reversalLevel: 0,
                    accumulatedLoss: 0,
                    takeProfit: parseFloat((assetConfig.INITIAL_STAKE * CONFIG.TAKE_PROFIT_MULTIPLIER).toFixed(2)),
                    takeProfitAmount: parseFloat((assetConfig.INITIAL_STAKE * CONFIG.TAKE_PROFIT_MULTIPLIER).toFixed(2)),

                    // Normal mode state (compat)
                    ...
                    // Multiplier reversal state
                    pendingReversal: null,

                    // Last analysis for notifications
                    lastAnalysis: null,
                };
                const mult = CONFIG.MULTIPLIER_VALUES[symbol] || 200;
                LOGGER.info(`Initialized asset: ${symbol} (Stake: $${assetConfig.INITIAL_STAKE}, Multiplier x${mult}, TP ${CONFIG.TAKE_PROFIT_MULTIPLIER}x, 15m)`);
            }
        });
    }
    ...
    handleSellResponse(r) {
        if (r.error) {
            LOGGER.error(`Sell (close) error: ${r.error.message}`);
            if (bot) bot._tradeLocked = false;
            // clear sellActive flag so next candle can retry
            const reqId = r.echo_req?.req_id;
            CONFIG.ACTIVE_ASSETS.forEach(sym => {
                const a = state.assets[sym];
                if (a?.activePositions) {
                    const pos = a.activePositions.find(p => p.sellReqId === reqId);
                    if (pos) pos.sellActive = false;
                }
            });
            return;
        }
        const sell = r.sell;
        LOGGER.trade(`Sell (close) initiated: contract ${sell.contract_id} sold price $${sell.sold_for ?? sell.price ?? '?'}`);
        // The actual P/L will arrive via proposal_open_contract is_sold update; keep sellActive until settled
    }
    ...
    handleOpenContract(r) {
        ...
        if (a.pendingReversal && !a.activePositions.length) {
            ...
            if (SessionManager.isSessionActive() && state.isAuthorized && !bot._tradeLocked) {
                ...
                if (prevCooldown > 0) {
                    LOGGER.info(`[${ownerSym}] Reversal entry bypassing cooldown ${prevCooldown} (opposite signal)`);
                    a.cooldownCandles = 0;
                }
                // For reversal, treat as normal WPR entry but preserve martingale stake (already updated via recordTradeResult)
                // If profit was loss, martingaleLevel already incremented; if win, reset — so use currentStake as is
                ...
                setImmediate(() => {
                    try {
                        ...
                        LOGGER.error(`[${ownerSym}] Reversal entry error: ${e.message}`);
                        if (bot) bot._forceReleaseTradeLock();
                    }
                });
            } else {
                LOGGER.warn(`[${ownerSym}] Pending reversal discarded — session inactive or trade locked`);
                a.pendingReversal = null;
            }
            // Do not run B recovery after reversal
            return;
        }
        ...
        if (isNewCandle) {
            ...
            if (closed.open_time !== a.lastProcessedCandleOpenTime) {
                ...
                if (!alreadyIn) {
                    ...
                    // ── v2 Persistent Breakout + Multiplier logic ──
                    if (a.activePositions.length > 0) {
                        // 1. Breakout reversal: BUY→SELL when close < low, SELL→BUY when close > high
                        const breakoutReversal = BreakoutManager.checkReversal(symbol);
                        if (breakoutReversal) {
                            const pos = a.activePositions[0];
                            const newDirInternal = breakoutReversal === 'UP' ? 'CALLE' : 'PUTE';
                            const newContract = breakoutReversal === 'UP' ? 'MULTUP' : 'MULTDOWN';
                            if (newDirInternal === 'CALLE') a.buyFlagActive = false; else a.sellFlagActive = false;
                            // Setup new persistent breakout levels for the reversal direction using current closed candle
                            const prevBreakoutSnap = { ...a.breakout };
                            const setupOk = BreakoutManager.setupBreakoutLevels(symbol, breakoutReversal === 'UP' ? 'UP' : 'DOWN', breakoutReversal === 'UP' ? 'BUY' : 'SELL');
                            if (!setupOk) {
                                // Restore if setup failed
                                a.breakout = prevBreakoutSnap;
                                LOGGER.warn(`[${symbol}] Breakout reversal setup failed — aborting reversal`);
                            } else {
                                a.pendingReversal = {
                                    direction: newDirInternal,
                                    contractType: newContract,
                                    analysis: { direction: newDirInternal, reason: `Breakout reversal ${pos.direction}→${newContract} close ${closed.close.toFixed(5)} beyond ${newDirInternal==='CALLE'?'high':'low'}`, details: { wpr: a.wpr, prevWpr: a.prevWpr, breakout: { ...a.breakout } } },
                                    reason: `Breakout reversal`,
                                    wpr: a.wpr, prevWpr: a.prevWpr,
                                    timestamp: Date.now()
                                };
                                LOGGER.signal(`[${symbol}] Breakout reversal ${pos.direction} → ${newContract} — closing and preparing reversal with new levels High ${a.breakout.highLevel.toFixed(5)} Low ${a.breakout.lowLevel.toFixed(5)}`);
                                bot.closeMultiplierPosition(symbol, `Breakout reversal`);
                            }
                        } else {
                            // 2. Check for WPR-driven breakout replacement during inTradeCycle (may also imply reversal)
                            const replacement = BreakoutManager.checkForBreakoutReplacement(symbol);
                            if (replacement) {
                                // Replacement already updated breakout levels; if it implies reversal direction, close and reverse
                                const curDir = a.currentDirection || a.activePositions[0]?.direction;
                                const isUpReplacement = replacement === 'UP';
                                const shouldReverse = (isUpReplacement && (curDir === 'DOWN' || curDir === 'MULTDOWN' || curDir === 'PUTE')) || (!isUpReplacement && (curDir === 'UP' || curDir === 'MULTUP' || curDir === 'CALLE'));
                                if (shouldReverse) {
                                    const newDirInternal = replacement === 'UP' ? 'CALLE' : 'PUTE';
                                    a.pendingReversal = {
                                        direction: newDirInternal,
                                        contractType: replacement === 'UP' ? 'MULTUP' : 'MULTDOWN',
                                        analysis: { direction: newDirInternal, reason: `WPR breakout replacement reversal`, details: { wpr: a.wpr, prevWpr: a.prevWpr } },
                                        reason: `WPR replacement`,
                                        wpr: a.wpr, prevWpr: a.prevWpr
                                    };
                                    bot.closeMultiplierPosition(symbol, `WPR breakout replacement`);
                                }
                            } else {
                                // 3. Fallback to opposite WPR signal close (original multiplier logic)
                                const pos = a.activePositions[0];
                                if (!pos.sellActive) {
                                    const closedBySignal = bot.maybeCloseOnOppositeSignal(symbol);
                                    if (closedBySignal) {
                                        LOGGER.trade(`[${symbol}] Opposite WPR signal — close initiated for ${pos.direction} (TP $${pos.takeProfit} SL $${pos.stopLoss} bypassed)`);
                                    } else {
                                        LOGGER.debug(`[${symbol}] Position open ${pos.direction} x${pos.multiplier}, no opposite/breakout signal — holding for TP $${pos.takeProfit} / SL $${pos.stopLoss}`);
                                    }
                                } else {
                                    LOGGER.debug(`[${symbol}] Sell already in progress for ${pos.direction}, awaiting settlement`);
                                }
                            }
                        }
                        a.canTrade = false;
                        ...
                        // No active position
                        // Check if waiting for re-entry after TP (persistent breakout)
                        if (a.waitingForReentry && a.breakout?.active) {
                            ...
                            if (reentry) {
                                ...
                                // Do not attempt WPR signals while waitingForReentry per SignalManager.updateWPRState
                                a.canTrade = false;
                                LOGGER.debug(`[${symbol}] Waiting for re-entry: price must return between ${a.breakout.lowLevel.toFixed(5)} - ${a.breakout.highLevel.toFixed(5)} then close beyond`);
                            }
                        } else {
                            a.canTrade = true;
    ...
    onClose() {
        ...
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
            ...
            TelegramService.sendMessage(`⚠️ <b>WILLMULT_v2 BOT CONNECTION LOST</b> — Reconnecting (attempt ${this.reconnectAttempts})`);
            ...
            TelegramService.sendMessage(`\u{1f6d1} <b>WILLMULT_v2 BOT STOPPED</b> — Max reconnections\nFinal P/L: $${(state.session.netPL || 0).toFixed(2)}`);
            process.exit(1);
        }
    }
...
class IndexBot {
    ...
    async start() {
        console.log('\n' + '═'.repeat(74));
        console.log(' DERIV MULTIPLIER BOT v3 — WILLIAMS %R ONLY (MULTUP/MULTDOWN)');
        ...
        LOGGER.info('WILLMULT_v2 BOT v1.0 fully started!');
    }
    ...
    // ── Duration helpers — not used for Multipliers (kept for compat) ──
    ...
    _getMultiplier(symbol) {
        return CONFIG.MULTIPLIER_VALUES[symbol] || CONFIG.MULTIPLIER_VALUES[symbol?.toUpperCase?.()] || 200;
    }

    _toMultiplierContract(direction) {
        // WPR signal uses CALLE/PUTE internally; multiplier market uses MULTUP/MULTDOWN
        if (direction === 'MULTUP' || direction === 'MULTDOWN') return direction;
        return direction === 'CALLE' ? 'MULTUP' : 'MULTDOWN';
    }

    _toInternalDirection(contractType) {
        if (contractType === 'MULTUP' || contractType === 'CALLE') return 'CALLE';
        if (contractType === 'MULTDOWN' || contractType === 'PUTE') return 'PUTE';
        return contractType;
    }
    ...
    _getTradeDuration(symbol) {
        // Multipliers have no duration — stay open until TP/SL or opposite signal
        return { duration: null, durationUnit: null, remaining: null };
    }
    ...
    _executeBuy(symbol, direction, isRecovery, analysis) {
        ...
        assetState.canTrade = false;
        assetState.lastTradeDirection = direction; // keep CALLE/PUTE for WPR logic
        assetState.currentDirection = contractType;

        const position = {
            symbol, direction: contractType, internalDirection: direction, stake, multiplier, takeProfit, stopLoss,
            duration: null, durationUnit: null,
        ...
        state.currentContractId = null;
        state.pendingTradeInfo = { symbol, stake, direction: contractType, internalDirection: direction, time: Date.now() };

        TelegramService.sendTradeAlert('OPEN', symbol, contractType, stake, null, null, {
            isRecovery,
            analysis: isRecovery ? null : analysis,
            multiplier, takeProfit, stopLoss
        });
    ...
    executeRecoveryTradeImmediate(symbol) {
        ...
        // Multiplier has no duration — just same-direction stake/martingale
        const mult = this._getMultiplier(symbol);
        LOGGER.trade(`🔄 [${symbol}] RECOVERY — ${this._toMultiplierContract(direction)} (WPR ${direction}) | Multiplier x${mult} | Stake $${a.currentStake.toFixed(2)} Martingale L${a.martingaleLevel}`);
        const pos = this._executeBuy(symbol, direction, true, null);
        if (pos) a.recoveryFirstDone = true;
        return !!pos;
    }

    // ── Multiplier opposite-signal close + reversal ──
    maybeCloseOnOppositeSignal(symbol) {
        const a = state.assets[symbol];
        if (!a || !a.activePositions.length) return false;
        const pos = a.activePositions[0];
        if (pos.sellActive) return false; // already closing
        if (!a.indicatorsReady || !Number.isFinite(a.wpr) || !Number.isFinite(a.prevWpr)) return false;
        // Determine opposite signal needed to close current position
        // MULTUP (CALLE) → close only on SELL signal (WPR cross below -80, first since overbought)
        // MULTDOWN (PUTE) → close only on BUY signal (WPR cross above -20, first since oversold)
        const activeContract = pos.direction; // MULTUP or MULTDOWN
        const activeInternal = pos.internalDirection || this._toInternalDirection(activeContract);
        let shouldClose = false;
        let reason = '';
        let oppositeSignal = null;
        let oppositeDirection = null;
        if (activeInternal === 'CALLE' || activeContract === 'MULTUP') {
            const sellSig = SignalManager.checkSellSignal(symbol);
            if (sellSig.shouldTrade) { shouldClose = true; reason = sellSig.reason; oppositeSignal = sellSig; oppositeDirection = 'PUTE'; }
        } else {
            const buySig = SignalManager.checkBuySignal(symbol);
            if (buySig.shouldTrade) { shouldClose = true; reason = buySig.reason; oppositeSignal = buySig; oppositeDirection = 'CALLE'; }
        }
        if (!shouldClose) return false;
        LOGGER.signal(`[${symbol}] Opposite WPR signal for ${activeContract} — ${reason} → closing multiplier and preparing reversal to ${oppositeDirection}`);
        // Consume the opposite flag for the new entry (first-cross logic) — will be re-armed only after revisiting extreme
        if (oppositeDirection === 'CALLE') a.buyFlagActive = false;
        else a.sellFlagActive = false;
        // Store pending reversal so that after close settles we immediately open opposite trade with same signal
        a.pendingReversal = {
            direction: oppositeDirection,
            contractType: this._toMultiplierContract(oppositeDirection),
            analysis: oppositeSignal,
            reason: reason,
            wpr: a.wpr,
            prevWpr: a.prevWpr,
            timestamp: Date.now()
        ...
        StatePersistence.saveState();
        return this.closeMultiplierPosition(symbol, reason);
    }

    closeMultiplierPosition(symbol, reason = '') {
        const a = state.assets[symbol];
        if (!a || !a.activePositions.length) return false;
        const pos = a.activePositions[0];
        if (!pos.contractId) {
            LOGGER.warn(`[${symbol}] Cannot close — contractId not yet assigned (buy pending)`);
            return false;
        }
        if (pos.sellActive) {
            LOGGER.debug(`[${symbol}] Close already in progress`);
            return false;
        }
        pos.sellActive = true;
        const reqId = this.connection.send({ sell: pos.contractId, price: 0 });
        pos.sellReqId = reqId;
        LOGGER.trade(`[${symbol}] Closing ${pos.direction} x${pos.multiplier} — contract ${pos.contractId} @ WPR ${a.prevWpr?.toFixed(2) ?? 'n/a'}→${a.wpr?.toFixed(2) ?? 'n/a'} ${reason}`);
        TelegramService.sendMessage(`🔒 <b>[${symbol}] Closing ${pos.direction} x${pos.multiplier}</b>\nWPR ${a.prevWpr?.toFixed(2) ?? 'n/a'}→${a.wpr?.toFixed(2) ?? 'n/a'}\nReason: ${reason}\nStake $${pos.stake.toFixed(2)}`);
        return true;
    }

    // ════════════════════════════════════════════════════════
    // CORE TRADE LOGIC — WPR ONLY (Multiplier)
    ...
    _recoverStuckTrade(reason) {
        ...
        TelegramService.sendMessage(
            `⚠️ <b>WILLMULT_v2 BOT STUCK TRADE RECOVERED [${reason}]</b>\n` +
            `Contract: ${contractId}\n` +
...
if (cliArgs.backtest) {
    ...
    console.log('\n\u{1f680} Starting WPR BOT v2.0 (Williams %R)...\n');
