...
class EnhancedDerivTradingBot {
    constructor(token, config = {}) {
        ...
        this.repetitionAnalysis = {
            windowSize: 100, // Size of sliding window for analysis
            minSampleSize: 50, // Minimum ticks needed for analysis
            assetRepetitionThresholds: {
                high: 0.25, // >25% repetition is considered high
                low: 0.05  // <5% repetition is considered low
            },
            digitRepetitionThresholds: {
                high: 0.35, // >35% repetition for specific digit
                low: 0.05   // <5% repetition for specific digit
            },
            confidenceScore: 0.6 // Minimum confidence to trade (0-1)
        };

        this.repetitionHistory = {
            assetRepetitions: [], // Array to track asset-wide repetitions
            digitFrequencies: Array(10).fill(0), // Count of each digit 0-9
            digitRepetitions: Array(10).fill(0), // Times each digit repeated
            lastDigit: null,
    ...
    checkRecentStreak(currentDigit, recentWindow) {
        if (recentWindow.length < 5) return { highProbability: false };
        
        // Check for recent repetitions of current digit
        let recentOccurrences = 0;
        let recentRepeats = 0;
        
        for (let i = 0; i < recentWindow.length; i++) {
            if (recentWindow[i] === currentDigit) {
                recentOccurrences++;
                if (i > 0 && recentWindow[i] === recentWindow[i - 1]) {
                    recentRepeats++;
                }
            }
        }
        
        const occurrenceRate = recentOccurrences / recentWindow.length;
        const repeatRate = recentOccurrences > 0 ? recentRepeats / recentOccurrences : 0;
        
        // If digit appeared frequently but didn't repeat much recently, high probability of change
        if (occurrenceRate > 0.3 && repeatRate < 0.2) {
            return { 
                highProbability: true, 
                type: 'Frequent but non-repeating',
                score: 0.8 
            };
        }
        
        // If digit appeared rarely but repeated when it did appear
        if (occurrenceRate < 0.15 && repeatRate > 0.4) {
            return { 
                highProbability: true, 
                type: 'Rare but repeating',
                score: 0.7 
            };
        }
        
        return { highProbability: false };
    }

    calculateMeanReversionProbability(currentDigit, digitCounts) {
        // Calculate expected uniform distribution (10% each digit)
        const expectedFrequency = 0.1;
        const totalOccurrences = digitCounts.reduce((a, b) => a + b, 0);
        
        if (totalOccurrences === 0) return 0;
        
        const currentFrequency = digitCounts[currentDigit] / totalOccurrences;
        const deviation = Math.abs(currentFrequency - expectedFrequency);
        
        // Higher deviation means higher probability of mean reversion
        // Using sigmoid function for smooth probability curve
        return 1 / (1 + Math.exp(-10 * (deviation - 0.1)));
    }

    analyzeRepetitionPatterns() {
        if (this.tickHistory.length < this.repetitionAnalysis.minSampleSize) {
            return {
                shouldTrade: false,
                confidence: 0,
        ...
        const windowSize = Math.min(this.repetitionAnalysis.windowSize, this.tickHistory.length - 1);
        const analysisWindow = this.tickHistory.slice(-windowSize - 1, -1);
        
        // Calculate asset-wide repetition rate
        let assetRepeats = 0;
        let digitCounts = Array(10).fill(0);
        let digitRepeats = Array(10).fill(0);
        
        for (let i = 1; i < analysisWindow.length; i++) {
            if (analysisWindow[i] === analysisWindow[i - 1]) {
                assetRepeats++;
                // Track which digit repeated
                digitRepeats[analysisWindow[i]]++;
            }
            digitCounts[analysisWindow[i]]++;
        }
        
        const assetRepetitionRate = assetRepeats / (analysisWindow.length - 1);
        const currentDigitFrequency = digitCounts[currentDigit] / analysisWindow.length;
        const currentDigitRepetitionRate = digitCounts[currentDigit] > 0 
            ? digitRepeats[currentDigit] / digitCounts[currentDigit] 
            : 0;
        ...
        // Factor 1: Asset-wide repetition rate
        if (assetRepetitionRate > this.repetitionAnalysis.assetRepetitionThresholds.high) {
            confidence += 0.2;
            reasons.push(`Asset repetition high (${(assetRepetitionRate*100).toFixed(1)}%)`);
        } else if (assetRepetitionRate < this.repetitionAnalysis.assetRepetitionThresholds.low) {
            confidence += 0.15;
            reasons.push(`Asset repetition low (${(assetRepetitionRate*100).toFixed(1)}%)`);
        }
        
        // Factor 2: Current digit repetition rate
        if (digitCounts[currentDigit] >= 5) { // Need minimum occurrences
            if (currentDigitRepetitionRate > this.repetitionAnalysis.digitRepetitionThresholds.high) {
                confidence += 0.25;
                reasons.push(`Digit ${currentDigit} repetition high (${(currentDigitRepetitionRate*100).toFixed(1)}%)`);
            } else if (currentDigitRepetitionRate < this.repetitionAnalysis.digitRepetitionThresholds.low) {
                confidence += 0.2;
                reasons.push(`Digit ${currentDigit} repetition low (${(currentDigitRepetitionRate*100).toFixed(1)}%)`);
            }
        }
        
        // Factor 3: Recent streak analysis
        const recentStreak = this.checkRecentStreak(currentDigit, analysisWindow.slice(-20));
        if (recentStreak.highProbability) {
            confidence += 0.15;
            reasons.push(`Recent pattern detected (${recentStreak.type})`);
        }
        
        // Factor 4: Mean reversion probability (digits tend to normalize over time)
        const meanReversionScore = this.calculateMeanReversionProbability(currentDigit, digitCounts);
        confidence += meanReversionScore * 0.1;
        
        // Cap confidence between 0 and 1
        confidence = Math.max(0, Math.min(1, confidence));
        
        return {
            shouldTrade: confidence >= this.repetitionAnalysis.confidenceScore,
            confidence: confidence,
            assetRepetitionRate: assetRepetitionRate,
            digitRepetitionRate: currentDigitRepetitionRate,
            currentDigit: currentDigit,
            digitFrequency: currentDigitFrequency,
    ...
    analyzeTicksEnhanced() {
        ...
        // Perform intelligent repetition analysis
        const analysis = this.analyzeRepetitionPatterns();

        this.winProbNumber = (analysis.confidence*100).toFixed(1);
        this.assetRepetitionRate = (analysis.assetRepetitionRate*100).toFixed(1);
        this.digitRepetitionRate = (analysis.digitRepetitionRate*100).toFixed(1);
        
        console.log(`📊 Analysis: Digit ${currentDigit} | Confidence: ${this.winProbNumber}%`);
        console.log(`   Asset Rep Rate: ${this.assetRepetitionRate}% | Digit Rep Rate: ${this.digitRepetitionRate}%`);
        ...
        if (analysis.shouldTrade  && this.winProbNumber >= 73) {
            ...
            console.log(`✅ HIGH CONFIDENCE - Placing trade on digit ${this.xDigit} | Confidence: ${this.winProbNumber}%`);
            console.log(`Analysis: ${analysis.reasons}`);
            
            this.placeTrade(this.xDigit, this.winProbNumber);
        } else {
            console.log(`⏸️  Skipping trade - Low confidence (${this.winProbNumber}%)`);
            // Reset trading state for next tick
            this.tradeInProgress = false;
        }
    }
    ...
    async sendLossEmail() {
        ...
        Asset Repetition Rate: ${this.assetRepetitionRate}
        Digit Repetition Rate: ${this.digitRepetitionRate}
