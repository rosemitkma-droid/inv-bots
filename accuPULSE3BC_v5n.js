 ...
 *  File: C:\Users\kenot\Desktop\Investment AccumulatorBot\accuPULSE3BC_v5n.js
 * =====================================================================
 *
 ...
 *  ✅ Offline selftest: node accuPULSE3BC_v5n.js --selftest
 */
...
// ── Configuration ──────────────────────────────────────────────────────
// Profile selector: 'aggressive' (~200 trades/day, surv 28.5/score 0.82) or
...
const PROFILE_DEFS = Object.freeze({
  ...
  conservative: Object.freeze({
    minNetEvRatio: 0.015,
    minPHorizon: 0.60,
    minConfidenceByRegime: { 0: 0.14, 1: 0.10, 2: 0.06, 3: 0.03 },
    minConfidence: 0.10,
    bestScore: 0.85,
    entryRequiredChecks: 5,
    minVolPercentile: 0.40,
    minEv: 0.015,
    minMomentum: 0.000025,
    minMomentum2: -0.000025,
    minSurvivalMean: 30,
    kellyFraction: 0.12,
    maxStakeMultiplier: 1.3,
    tpProfitPct: 0.30,
  }),
});
function resolveProfile(base, profileName) {
  const def = PROFILE_DEFS[profileName] || PROFILE_DEFS.aggressive;
  // deep-clone base and apply profile overrides
  const cfg = JSON.parse(JSON.stringify(base));
  cfg.minNetEvRatio = def.minNetEvRatio;
  cfg.boomCrash = cfg.boomCrash || {};
  cfg.boomCrash.minPHorizon = def.minPHorizon;
  cfg.minConfidenceByRegime = { ...def.minConfidenceByRegime };
  cfg.minConfidence = def.minConfidence;
  cfg.bestScore = def.bestScore;
  cfg.kellyFraction = def.kellyFraction;
  cfg.maxStakeMultiplier = def.maxStakeMultiplier;
  // entryConfirmation
  cfg.entryConfirmation = cfg.entryConfirmation || {};
  cfg.entryConfirmation.requiredChecks = def.entryRequiredChecks;
  cfg.entryConfirmation.minVolPercentile = def.minVolPercentile;
  cfg.entryConfirmation.minEv = def.minEv;
  cfg.entryConfirmation.minMomentum = def.minMomentum;
  cfg.entryConfirmation.minMomentum2 = def.minMomentum2;
  cfg.entryConfirmation.minSurvivalMean = def.minSurvivalMean;
  // tp_tiers quick_win
  cfg.tp_tiers = cfg.tp_tiers || {};
  cfg.tp_tiers.quick_win = cfg.tp_tiers.quick_win || {};
  cfg.tp_tiers.quick_win.profit_pct = def.tpProfitPct;
  return cfg;
}
const _BASE_CONFIG = {
  ...
  logFile: 'accuPULSE3BC_v5n_0004.log',
  logLevel: 'INFO3BC_v5n',
  stateFile: 'accuPULSE3BC_state_v5n_0004.json',
  metricsFile: 'metricsBC_v5n_0004.json',
  metricsFileV5: 'accuPULSE3BC_analysis_v5n_0004.jsonl',  // Feature 7: Full metrics logging
...
class EnhancedTradeExecutor extends EventEmitter {
  ...
  computeStake(baseStake, compositeScore, volHurst, recentWinRate, kellyEdge) {
    ...
    if (volHurst > this.cfg.volAdjustThreshold) {
      ...
      telegram.send(`⚠️ <b>AccuPULSE3BC_v5n Volatility spike detected</b>\nHurst: ${volHurst.toFixed(2)}\nStake: -40%`);
    }
    let wrMult = 1.0;
...
class AccuPULSE3BotV5 {
  ...
  _maybeResetDailyState() {
    ...
    try { telegram.send(`🔄 <b>AccuPULSE3BC_v5n Daily reset</b> ${today} → WARMUP (Day-1 frequency restored)`); } catch (_) {}
  ...
  updateRecovery() {
    ...
    if (this.lossStreak >= rc.triggerLosses && !this.recoveryMode) {
      ...
      // telegram.send(
      //   `🔄 <b>AccuPULSE3BC_v5n Recovery mode activated</b>\n` +
    ...
    if (this.recoveryMode) {
      ...
      if (this.recoveryWins >= (rc.recoveryExitWins || 10)) {
        ...
        telegram.send('✅ <b>AccuPULSE3BC_v5n Recovery mode exited</b>\n10 wins achieved');
        return;
      }
  ...
  async start() {
    log('INFO', '═══════════════════════════════════════════');
    log('INFO', '  AccuPULSE3 v5.1-BC — Gates-Optimized (profile selectable)');
    log('INFO', '═══════════════════════════════════════════');
    log('INFO', `Profile: ${this.cfg.profile} (aggressive=surv28.5/score0.82/4checks | conservative=surv30/score0.85/5checks) — edit ACTIVE_PROFILE at top of file to switch`);
  ...
  async _onAuthorized(info) {
    if (this.cfg.demoOnly && !info.isVirtual) {
      ...
      this.stopped = true;
      telegram.send('AccuPULSE3BC_v5n STOPPED: demoOnly is enabled but the authorized account is not virtual.');
      this.client.stop();
      return;
    }
  ...
  _onDisconnected(code, reason, wasAuth) {
    if (this._stuckCheckTimer) clearInterval(this._stuckCheckTimer);
    if (this._metricsTimer) clearInterval(this._metricsTimer);
    telegram.send(`⚠️ <b>AccuPULSE3BC_v5n Connection lost</b>\ncode: <code>${code}</code>\nwas auth: ${wasAuth ? 'yes' : 'no'}\n🔄 reconnecting…`);
    if (this._analysisT) {
  ...
  _onTradeOpen(t) {
    try {
      ...
      const a = t._analysis;
      let msg =
        `🟢 <b>AccuPULSE3BC_v5n TRADE OPENED</b>\n\n` +
        `🎫 <b>#</b>${t.contractId}\n` +
        `📊 <code>${t.symbol}</code>\n` +
      ...
      try {
        ...
        try { telegram.send(`🟢 <b>AccuPULSE3BC_v5n TRADE OPENED</b> #${t.contractId} ${t.symbol} stake=${t.stake}`); } catch(_) {}
      }
      ...
      try {
        const fallback = `🟢 <b>AccuPULSE3BC_v5n TRADE OPENED (fallback)</b>\n🎫 #${t.contractId ?? '—'} ${t.symbol ?? ''}\nStake: ${t.stake ?? '—'}`;
  ...
  _onTradeResult(t) {
    ...
    let msg =
      ` AccuPULSE3BC_v5n ${emoji} <b>TRADE ${t.status === 'won' ? 'WON' : (t.status === 'unknown' ? 'UNKNOWN' : 'LOST')}</b>\n\n` +
      `🎫 #${t.contractId} | ${t.symbol}\n` +
  ...
  _checkCircuitBreakers() {
    ...
    if (pl >= 5000) { log('WARN', `AccuPULSE3BC_v5n Session profit limit`); return true; }
    if (pl <= -this.cfg.dailyMaxLoss) { telegram.send(`🛑 AccuPULSE3BC_v5n Daily loss limit`); return true; }
    if (today.length >= this.cfg.dailyMaxTrades) { telegram.send(`🛑 AccuPULSE3BC_v5n Daily trade limit`); return true; }
    const dd = this.equityPeak > 0 ? (this.equityPeak - (this.lastBalance ?? 0)) / this.equityPeak : 0;
    if (dd > 0.50) { telegram.send(`🛑 AccuPULSE3BC_v5n DD limit (50%)`); return true; }
    if (this.lossStreak >= this.cfg.streakStopDay) { telegram.send(`🛑 AccuPULSE3BC_v5n Loss streak limit`); return true; }
    return false;
  }
  ...
  async _analyzeAndTrade() {
    ...
    try {
      ...
      if (this._prevTopAsset !== newTopAsset) {
        // telegram.send(`📊 <b>AccuPULSE3BC_v5n Asset Rotation</b>\n${this._prevTopAsset || '—'} → ${newTopAsset}`);
        this._prevTopAsset = newTopAsset;
      }
    } catch (e) {
  ...
  stop(signal) {
    ...
    telegram.send(`🛑 <b>AccuPULSE3BC_v5n stopped</b>\nSignal: ${signal}`);
    if (this._analysisT) clearInterval(this._analysisT);
    ...
    telegram.send(
      `🌙 <b>AccuPULSE3BC_v5n SESSION END</b>\n` +
      `📊 ${s.count} trades | WR ${s.winRate.toFixed(1)}%\n` +
      `💰 ${money(s.totalProfit, this.currencyStr())}\n` +
...
function selftest() {
  ...
  t('Classic hazard path still works for non-BC symbol', () => {
    ...
    const m = analyzer._hazardEstimate(ticks, 0.10, 0.03, 'R_10'); // 3% growth ensures EV > 1% threshold (aggressive profile)
