...
const TELEGRAM_TOKEN = "7919033379:AAHluKFMECmhMrBhNr_XVpWvCKEonQPx9_0"; // Telegram bot token
const CHAT_ID = "752497117";

// ============================================
// STATE PERSISTENCE MANAGER
// ============================================
const STATE_FILE = path.join(__dirname, 'dubaiPrince-state.json');
...
class BlackFibonacci {
    ...
    detectPhoenixSignal() {
        ...
        for (const len of fib) {
            ...
            slice.forEach(x => c[x]++);
            const exp = len / 10;
            const sd = Math.sqrt(len * 0.1 * 0.9);
            for (let i = 0; i < 10; i++) z[i] += (c[i] - exp) / sd;
        }

        let bestZ = 0, best = 0;
        for (let i = 0; i < 10; i++) {
            if (z[i] > bestZ) { bestZ = z[i]; best = i; }
        }

        // ULTRA-LOW VOL + DOMINANCE CHECK
