...
/**
 * ╔══════════════════════════════════════════════════════════════════════════╗
 * ║   DERIV SYNTHETIC INDICES MULTIPLIER BOT — v3  "WILLIAMS %R ONLY"        ║
 * ║  MARKET: MULTIPLIERS (MULTUP/MULTDOWN) — from Rise/Fall WPR base         ║
 * ║  STRATEGY:                                                               ║
 * ║  WPR_PERIOD (default 7, configurable via CONFIG.WPR_PERIOD)              ║
 * ║  BUY:  WPR crosses above -20 (prev <= -20 → cur > -20) — FIRST cross     ║
 * ║        since oversold (-80) → MULTUP                                     ║
...
const CONFIG = {
    ...
    TIMEFRAME_LABEL: '1m',
    CANDLES_TO_LOAD: 150,
    MAX_CANDLES_STORED: 250,
    // NOTE: DURATION removed for multipliers — positions stay open until TP/SL or opposite signal
    ...
    initializeAssets() {
        CONFIG.ACTIVE_ASSETS.forEach(symbol => {
            if (!state.assets[symbol]) {
                ...
                LOGGER.info(`Initialized asset: ${symbol} (Stake: $${assetConfig.INITIAL_STAKE}, Multiplier x${mult}, TP ${CONFIG.TAKE_PROFIT_MULTIPLIER}x)`);
            }
        });
    }
        ...
        if (isNewCandle) {
            ...
            if (closed.open_time !== a.lastProcessedCandleOpenTime) {
                ...
                if (!alreadyIn) {
                    ...
                    // ── Multiplier close logic: if position open, only close on opposite WPR signal, else hold for TP/SL ──
                    if (a.activePositions.length > 0) {
                        // Position is open — check for opposite-signal close
                        ...
                        if (!pos.sellActive) {
                            ...
                            if (closedBySignal) {
                                ...
                                LOGGER.debug(`[${symbol}] Position open ${pos.direction} x${pos.multiplier}, no opposite signal — holding for TP $${pos.takeProfit} / SL $${pos.stopLoss}`);
                            }
                        } else {
