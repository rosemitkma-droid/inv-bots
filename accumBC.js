...
// ══════════════════════════════════════════════════════════════════════════════
const STATE_FILE = path.join(__dirname, 'accumBC_008_state.json');
...
class EnhancedDerivTradingBot {
    constructor(token, config = {}) {
        ...
        this.waitTime = 50000; // Reconnect after Waiting 
        this.LossDigitsList = [];
    ...
    suspendOtherAssets(lossAsset) {
        ...
        //     `🔒 <b>Asset Suspension (Accum BOOM)</b>\n\n` +
    ...
    resumeAllAssets() {
        ...
        // this.sendTelegramMessage(
        //     `✅ <b>All Assets Resumed (Accum BOOM)</b>\n\n` +
    ...
    async sendHourlySummary() {
        ...
        await this.sendTelegramMessage(
            `📊 <b>Session Summary Accum BOOM</b>\n\n` +
    ...
    handleProposal(message) {
        ...
        const condition =  this.checkTradeCondition(stayedInArray, this.consecutiveLosses, 1600) && this.checkTradeCondition2(stayedInArray2, this.consecutiveLosses, 30); 
    ...
    _recoverStuckTrade(reason) {
        ...
        this.sendTelegramMessage(
            `🚨 <b>STUCK TRADE RECOVERED Accum BOOM[${reason}]</b>\n\n` +
            `Contract: ${contractId}\n` +
            `Asset: ${stuckAsset}\n` +
            `Stake: $${stake.toFixed(2)}\n` +
            `Open for: ${openSeconds}s\n` +
