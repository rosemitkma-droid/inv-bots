...
class TelegramService {
    ...
    static async sendTradeAlert(type, symbol, stake, profitDetails = {}) {
        ...
        const lines = [
            `${emoji} <b>ACCUMULATOR BOT3 v1 — ${type}</b>`,
            `Asset: <b>${symbol}</b>`,
...
class ConnectionManager {
    ...
    onClose() {
        ...
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
            ...
            TelegramService.sendMessage(`⚠️ <b>BOT3 CONNECTION LOST</b> — Reconnecting (attempt ${this.reconnectAttempts})`);
            setTimeout(() => { this.isReconnecting = false; this.connect(); }, delay);
        } else {
            LOGGER.error('Max reconnection attempts reached — giving up');
            TelegramService.sendMessage(`🛑 <b>BOT3 STOPPED</b> — Max reconnections\nFinal P/L: $${state.session.netPL.toFixed(2)}`);
            process.exit(1);
        }
    }
...
class AccumulatorBot {
    ...
    async start() {
        ...
        await TelegramService.sendMessage(
            `🤖 <b>ACCUMULATOR BOT3 v1.1 STARTED</b>\n` +
        ...
        setInterval(() => {
            ...
            if (lastWinAsset && !state.session.endedByTime) {
                if (hour >= CONFIG.DISCONNECT_HOUR && mins >= 0) {
                    ...
                    TelegramService.sendMessage(`🌙 <b>BOT3 Time Stop</b> — ${CONFIG.DISCONNECT_HOUR}:00 reached after win. Bot paused.`);
                    this.stop();
                }
            }
