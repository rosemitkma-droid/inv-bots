...
// FILE PATHS  [RETAINED]
// ============================================================
const STATE_FILE = path.join(__dirname, 'bizWillRFv3_002-state.json');
const HISTORY_FILE = path.join(__dirname, 'bizWillRFv3_002-history.json');
...
// ============================================================
// SIGNAL MANAGER v3 — EVERY valid cross + confirmation-candle filter.
...
class SignalManager {
    static seedWPRState(symbol) {
        ...
        // Legacy flags kept for state compat — always false in v3 (unused).
    ...
    static updateWPRState(symbol) {
        // v3: no arming — cross + candle filter only. Keep flags cleared.
        const a = state.assets[symbol];
        if (!a) return false;
        a.buyFlagActive = false;
        a.sellFlagActive = false;
    ...
    static analyze(symbol, confirmingCandle = null) {
        const buySig = this.checkBuySignal(symbol, confirmingCandle);
...
class BacktestEngine {
    ...
    async run(symbol, candles, opts={}) {
        ...
        // v3: no arm flags — cross + candle filter only.
        ...
        for (let i = 0; i < candles.length; i++) {
            ...
            // v3: no flag consumption — cross + candle filter only.
    ...
    formatReport(r){
        ...
        return [
            `🧪 BACKTEST v3 cross+candle — ${r.symbol} (WPR=${r.wprPeriod ?? r.WPR_PERIOD ?? CONFIG.WPR_PERIOD} cross >${CONFIG.WPR_OVERSOLD}→CALLE / cross <${CONFIG.WPR_OVERBOUGHT}→PUTE, ${r.granularity}s, payout ${(r.payoutRatio*100).toFixed(0)}%, stake $${getAssetConfig(r.symbol).INITIAL_STAKE}×${getAssetConfig(r.symbol).MARTINGALE_MULTIPLIER}, pool $${getAssetConfig(r.symbol).INVESTMENT_AMOUNT})`,
...
class TelegramService {
    ...
    static async sendTradeAlert(type, symbol, direction, stake, duration, durationUnit, details = {}) {
        ...
        if (type === 'OPEN' && details) {
            ...
            if (details.isRecovery) {
                analysisDetails = `
        🔄 <b>BizWillRFv3 SIGNAL-WAIT RECOVERY L${a?.martingaleLevel ?? 0}</b> (new WPR signal direction, stake multiplier only)
        ...
        📊 Signal: ${analysis?.direction || direction} (${analysis?.reason || ''})`;
                ...
                analysisDetails = `
        🧠 <b>BizWillRFv3 WPR(${CONFIG.WPR_PERIOD}) Signal:</b>
        📊 WPR: ${wprStr} (OB ${CONFIG.WPR_OVERBOUGHT} / OS ${CONFIG.WPR_OVERSOLD})
        ...
        const msg = `
        ${emoji} <b>${type} BizWillRFv3 TRADE ALERT - ${recoveryStatus}</b>
    ...
    static async sendHourlySummary() {
        ...
        await this.sendMessage([
            `⏰ <b>BizWillRFv3 HOURLY SUMMARY (multi-asset independent)</b>`,
    ...
    static async sendSessionSummary() {
        ...
        await this.sendMessage([
            `\u{1f4ca} <b>BizWillRFv3 SESSION SUMMARY (independent)</b>`,
...
class SessionManager {
    ...
    static checkAssetTargets(symbol) {
        ...
        if (Number.isFinite(sl) && a.netPL <= sl) {
            ...
            TelegramService.sendMessage(`🛑 <b>[${symbol}] BizWillRFv3 STOP-LOSS</b>\nP/L: $${a.netPL.toFixed(2)}\nPool: $${a.investmentRemaining.toFixed(2)} — others continue`);
            return true;
        }
        return false;
    }
    ...
    static checkDayChange() {
        ...
        if (state.currentTradeDay && state.currentTradeDay !== today) {
            ...
            TelegramService.sendMessage(
                `\u{1f319} <b>BizWillRFv3 END OF DAY ${state.currentTradeDay}</b>\nP/L: $${(dayStats?.netPL || 0).toFixed(2)}\nCapital: $${state.capital.toFixed(2)}`
            );
...
class ConnectionManager {
    ...
    handleBuyResponse(r) {
        ...
        if (!matched) {
            ...
            TelegramService.sendMessage(
                `🚨 <b>BizWillRFv3 UNTRACKED CONTRACT ${contract.contract_id}</b>\n` +
    ...
    handleOpenContract(r) {
        ...
        if (stuckIdx >= 0) {
            ...
            TelegramService.sendMessage(
                `🚨 <b>BizWillRFv3 LATE SETTLEMENT ${contractIdStr} (${entry.symbol})</b>\n` +
    ...
    onClose() {
        ...
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
            ...
            TelegramService.sendMessage(`⚠️ <b>BizWillRFv3 BOT CONNECTION LOST</b> — Reconnecting (attempt ${this.reconnectAttempts})`);
            ...
            TelegramService.sendMessage(`\u{1f6d1} <b>BizWillRFv3 BOT STOPPED</b> — Max reconnections\nFinal P/L: $${(state.session.netPL || 0).toFixed(2)}`);
            process.exit(1);
        }
    }
...
class IndexBot {
    ...
    async start() {
        ...
        LOGGER.info('WILLRFv3 BOT fully started!');
    }
    ...
    _startBuyAckTimeout(symbol, reqId) {
        ...
        const timer = setTimeout(() => {
            ...
            TelegramService.sendMessage(
                `⚠️ <b>[${symbol}] BizWillRFv3 BUY ACK TIMEOUT</b>\n` +
    ...
    executeNextTrade(symbol, lastClosedCandle) {
        ...
        // v3: the just-closed candle is the confirmation candle — cross alone is not enough.
        const analysis = SignalManager.analyze(symbol, lastClosedCandle);
        ...
        const confDir = analysis.details?.candleClose != null && analysis.details?.candleOpen != null
            ? (analysis.details.candleClose < analysis.details.candleOpen ? 'BEARISH' : 'BULLISH')
            : 'n/a';
        if (isRecovery) {
            LOGGER.trade(`🔄 [${symbol}] RECOVERY SIGNAL L${assetState.martingaleLevel} WPR(${wprPeriod}) ${analysis.details.prevWpr?.toFixed(2) ?? ''}→${analysis.details.wpr?.toFixed(2) ?? ''} → ${direction} (signal direction, NOT same-direction, confirm ${confDir}) | ${analysis.reason}`);
        } else {
            LOGGER.trade(`🎯 [${symbol}] WPR v3 SIGNAL WPR(${wprPeriod}) ${analysis.details.prevWpr?.toFixed(2) ?? ''}→${analysis.details.wpr?.toFixed(2) ?? ''} → ${direction} (confirm ${confDir}) | ${analysis.reason}`);
        }
        // v3: no flag consumption — every valid cross + candle filter trades.
    ...
    _recoverStuckTrade(reason, contractId = null) {
        ...
        TelegramService.sendMessage(
            `⚠️ <b>BizWillRFv3 BOT STUCK TRADE [${reason}]</b>\n` +
            (last
...
async function runBacktestCLI(opts) {
    ...
    for(const sym of symbols){
        ...
        const fname = `bizWillRFv3-backtest-${sym}-${new Date().toISOString().slice(0,10).replace(/-/g,'')}.json`;
...
if (cliArgs.backtest) {
    ...
    console.log('\n\u{1f680} Starting WPR BOT v3 CROSS+CANDLE (Williams %R)...\n');
