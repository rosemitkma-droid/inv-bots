 ...
 * ║  SELL:  WPR crosses above -20 (prev <= -20 → cur > -20) — FIRST cross     ║
 * ║        since coming from oversold (-80) → CALLE (Rise)                   ║
 * ║  BUY: WPR crosses below -80 (prev >= -80 → cur < -80) — FIRST cross     ║
...
// FILE PATHS  [RETAINED]
// ============================================================
const STATE_FILE = path.join(__dirname, 'bizWillRFb_01-state.json');
const HISTORY_FILE = path.join(__dirname, 'bizWillRFb_01-history.json');
...
class SignalManager {
    ...
    static checkBuySignal(symbol) {
        ...
        if (!a.buyFlagActive) {
            return { shouldTrade: false, reason: `SELL cross ${prevWpr.toFixed(2)}→${wpr.toFixed(2)} but SELL flag not armed (never visited ${CONFIG.WPR_OVERSOLD} since last SELL)`, details: { wpr, prevWpr } };
        }
        return {
            shouldTrade: true,
            direction: 'PUTE',
            confidence: 1,
            reason: `WPR SELL cross ${prevWpr.toFixed(2)}→${wpr.toFixed(2)} above ${CONFIG.WPR_OVERBOUGHT} (first since oversold)`,
    ...
    static checkSellSignal(symbol) {
        ...
        if (!a.buyFlagActive) {
            return { shouldTrade: false, reason: `BUY cross ${prevWpr.toFixed(2)}→${wpr.toFixed(2)} but BUY flag not armed (never visited ${CONFIG.WPR_OVERBOUGHT} since last BUY)`, details: { wpr, prevWpr } };
        }
        return {
            shouldTrade: true,
            direction: 'CALLE',
            confidence: 1,
            reason: `WPR BUY cross ${prevWpr.toFixed(2)}→${wpr.toFixed(2)} below ${CONFIG.WPR_OVERSOLD} (first since overbought)`,
            details: { wpr, prevWpr, buyFlagActive: a.buyFlagActive, sellFlagActive: a.sellFlagActive }
        };
    }
...
class BacktestEngine {
    ...
    async run(symbol, candles, opts={}) {
        ...
        for (let i = 0; i < candles.length; i++) {
            ...
            const sellCross = prevWpr <= CONFIG.WPR_OVERBOUGHT && wpr > CONFIG.WPR_OVERBOUGHT && buyFlagActive;
            const buyCross = prevWpr >= CONFIG.WPR_OVERSOLD && wpr < CONFIG.WPR_OVERSOLD && sellFlagActive;
            let direction = null;
...
class TelegramService {
    ...
    static async sendStartupMessage() {
        ...
        await this.sendMessage([
            ...
            `Strategy: Williams %R(${CONFIG.WPR_PERIOD}) cross ${CONFIG.WPR_OVERBOUGHT}/${CONFIG.WPR_OVERSOLD} → PUTE/CALLE (signal always required)`,
...
class SessionManager {
    ...
    static checkAssetTargets(symbol) {
        ...
        if (Number.isFinite(sl) && a.netPL <= sl) {
            ...
            TelegramService.sendMessage(`🛑 <b>[${symbol}] BizWillRFb STOP-LOSS</b>\nP/L: $${a.netPL.toFixed(2)}\nPool: $${a.investmentRemaining.toFixed(2)} — others continue`);
            return true;
        }
        return false;
    }
    ...
    static checkDayChange() {
        ...
        if (state.currentTradeDay && state.currentTradeDay !== today) {
            ...
            TelegramService.sendMessage(
                `\u{1f319} <b>BizWillRFb BOT END OF DAY ${state.currentTradeDay}</b>\nP/L: $${(dayStats?.netPL || 0).toFixed(2)}\nCapital: $${state.capital.toFixed(2)}`
            );
...
class ConnectionManager {
    ...
    handleBuyResponse(r) {
        ...
        if (!matched) {
            ...
            TelegramService.sendMessage(
                `🚨 <b>BizWillRFb UNTRACKED CONTRACT ${contract.contract_id}</b>\n` +
    ...
    handleOpenContract(r) {
        ...
        if (stuckIdx >= 0) {
            ...
            TelegramService.sendMessage(
                `🚨 <b>BizWillRFb LATE SETTLEMENT ${contractIdStr} (${entry.symbol})</b>\n` +
    ...
    onClose() {
        ...
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
            ...
            TelegramService.sendMessage(`⚠️ <b>BizWillRFb BOT CONNECTION LOST</b> — Reconnecting (attempt ${this.reconnectAttempts})`);
            ...
            TelegramService.sendMessage(`\u{1f6d1} <b>BizWillRFb BOT STOPPED</b> — Max reconnections\nFinal P/L: $${(state.session.netPL || 0).toFixed(2)}`);
            process.exit(1);
        }
    }
...
class IndexBot {
    ...
    async start() {
        ...
        console.log(`WPR       : Period=${CONFIG.WPR_PERIOD} OB=${CONFIG.WPR_OVERBOUGHT} OS=${CONFIG.WPR_OVERSOLD} | BUY: cross >-20 (first since -80) → PUTE | SELL: cross <-80 (first since -20) → CALLE`);
    ...
    _startBuyAckTimeout(symbol, reqId) {
        ...
        const timer = setTimeout(() => {
            ...
            TelegramService.sendMessage(
                `⚠️ <b>[${symbol}] BizWillRFb BUY ACK TIMEOUT</b>\n` +
    ...
    _recoverStuckTrade(reason, contractId = null) {
        ...
        TelegramService.sendMessage(
            `⚠️ <b>BizWillRFb BOT STUCK TRADE [${reason}]</b>\n` +
            (last
